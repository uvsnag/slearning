...
export const topics: PvTopic[] = [
  {
    ...
    icon: '☕',
    questions: [
      // ──── 1. CORE JAVA FUNDAMENTALS — JVM, syntax, strings, numbers ────
      {
...
<details class="viet-answer"><summary>🇻🇳 Đáp án (Tiếng Việt)</summary><p><strong>JVM</strong> là bộ máy chạy Java bytecode, và nó khác nhau tùy theo từng hệ điều hành. <strong>JRE</strong> là JVM cộng với các thư viện chuẩn cần thiết để chạy một chương trình. <strong>JDK</strong> là JRE cộng với các công cụ phát triển như <code>javac</code> và trình gỡ lỗi. Bạn phát triển bằng JDK, và chính cách phân lớp này làm nên tinh thần 'write once, run anywhere'.</p></details>
<ul>
...
<details class="viet-answer"><summary>🇻🇳 Đáp án (Tiếng Việt)</summary><p><code>var</code> cho phép trình biên dịch tự suy ra kiểu của biến cục bộ từ giá trị của nó. Kiểu vẫn cố định và được kiểm tra tại thời điểm biên dịch; đây không phải kiểu động như trong JavaScript. Hãy dùng nó khi kiểu đã rõ ràng từ vế phải, chẳng hạn với các kiểu generic dài, để code ngắn gọn hơn. Tránh dùng khi kiểu bị che giấu, ví dụ <code>var x = getResult()</code>. Nó chỉ dùng được cho biến cục bộ, không dùng cho trường, tham số hay kiểu trả về.</p></details>
...
<details class="viet-answer"><summary>🇻🇳 Đáp án (Tiếng Việt)</summary><p>Ba từ này chỉ nghe giống nhau thôi. <code>final</code> là một modifier: một biến final không thể được gán lại, một method final không thể bị override, và một class final không thể được kế thừa. <code>finally</code> là một khối sau try/catch luôn luôn chạy, dùng để dọn dẹp. <code>finalize()</code> là một method cũ, không đáng tin cậy của garbage collection và đã bị deprecated từ Java 9, nên hãy dùng try-with-resources thay thế. Lưu ý rằng <code>finally</code> vẫn chạy ngay cả khi try có return, nhưng không chạy nếu JVM thoát bằng <code>System.exit</code>.</p></details>
...
<details class="viet-answer"><summary>🇻🇳 Đáp án (Tiếng Việt)</summary><p><code>this</code> trỏ tới đối tượng hiện tại. Nó được dùng để phân biệt một field với một tham số, hoặc để gọi một constructor khác bằng <code>this(...)</code>. <code>super</code> trỏ tới lớp cha. Nó được dùng để gọi một method của cha đã bị override, hoặc gọi constructor của cha bằng <code>super(...)</code>. Một lời gọi <code>this(...)</code> hoặc <code>super(...)</code> phải là dòng đầu tiên trong constructor, và nếu bạn không viết cái nào cả, Java sẽ tự thêm một <code>super()</code> không tham số cho bạn.</p></details>
<ul>
...
<details class="viet-answer"><summary>🇻🇳 Đáp án (Tiếng Việt)</summary><p>Java luôn luôn là pass-by-value, không có ngoại lệ. Với đối tượng, giá trị được truyền đi là một bản sao của tham chiếu, chứ không phải bản thân đối tượng. Bên trong một method, bạn có thể thay đổi đối tượng thông qua tham chiếu đó và bên gọi sẽ thấy được, vì cả hai cùng trỏ tới một đối tượng. Nhưng nếu bạn gán lại tham số cho một đối tượng mới thì biến của bên gọi không đổi. Bằng chứng: một method <code>swap</code> không thể hoán đổi các biến của bên gọi, nhưng <code>list.add(x)</code> thì lại thấy được từ bên ngoài. Thay đổi trên đối tượng thì hiện ra; còn gán lại thì không.</p></details>
...
<details class="viet-answer"><summary>🇻🇳 Đáp án (Tiếng Việt)</summary><p><code>String</code> là <strong>immutable</strong>, nên mỗi lần thay đổi lại tạo ra một object mới. <code>StringBuilder</code> thì mutable và nhanh, nhưng không thread-safe. <code>StringBuffer</code> là phiên bản thread-safe cũ hơn và ngày nay hiếm khi cần đến. Lỗi phổ biến là nối chuỗi bằng <code>+</code> bên trong vòng lặp, việc này rất chậm; hãy dùng <code>StringBuilder</code> ở đó thay thế.</p></details>
<ul>
...
<details class="viet-answer"><summary>🇻🇳 Đáp án (Tiếng Việt)</summary><p><strong>String pool</strong> là một vùng cache chứa các string literal duy nhất trên heap, nên các literal giống nhau sẽ dùng chung một object. Đó là lý do <code>"a" == "a"</code> trả về true nhưng <code>new String("a") == "a"</code> lại là false. <code>intern()</code> đưa một string vào pool và trả về instance dùng chung, nhưng nó hiếm khi cần đến và có thể gây hại vì các string đã intern tồn tại rất lâu. Quy tắc quan trọng là luôn so sánh string bằng <code>equals()</code>, không bao giờ dùng <code>==</code>.</p></details>
...
<details class="viet-answer"><summary>🇻🇳 Đáp án (Tiếng Việt)</summary><p>Vì <code>double</code> dùng dấu phẩy động nhị phân, các giá trị như 0.1 không thể lưu chính xác. Nên <code>0.1 + 0.2</code> cho ra 0.30000000000000004, và những sai số nhỏ này dồn lại thành số tiền sai. Với tiền tệ hãy dùng <code>BigDecimal</code>, và tạo nó từ một <strong>String</strong>: dùng <code>new BigDecimal("0.1")</code>, đừng bao giờ dùng <code>new BigDecimal(0.1)</code> vì nó sao chép luôn cả sai số đó. Ngoài ra hãy đặt scale rõ ràng và <code>RoundingMode</code> khi chia, vì nếu không <code>BigDecimal</code> sẽ ném lỗi khi kết quả không kết thúc (non-terminating). Quy tắc: đừng bao giờ dùng <code>float</code> hay <code>double</code> cho tiền.</p></details>
...
<div class="key-point">Money options in production: <code>BigDecimal</code> (string constructor + explicit <code>RoundingMode</code>), or store <strong>long cents</strong> (minor units) — what payment systems actually do. Bonus trivia: <code>double</code> can't represent 0.1 exactly for the same reason 1/3 isn't exact in decimal.</div>`,
      ...
      {
        q: 'Cheat sheet: how do you convert between the common Java types (String, primitives, arrays, List)?',
        difficulty: 'easy',
        a: `<div class="interview-answer"><p>Most conversions follow three families. <strong>Anything to String</strong> uses <code>String.valueOf(x)</code> — it is null-safe for objects, unlike <code>x.toString()</code>. <strong>String to a primitive</strong> uses the wrapper's parse method: <code>Integer.parseInt</code>, <code>Double.parseDouble</code>, <code>Float.parseFloat</code>, all of which throw <code>NumberFormatException</code> on bad input. <strong>Array and List</strong> convert with <code>Arrays.asList(arr)</code> and <code>list.toArray(new String[0])</code>. The one place this breaks down is primitive arrays: <code>int[]</code> is not <code>Integer[]</code>, so <code>Arrays.asList(intArray)</code> gives you a single-element list holding the array. Use a stream with <code>boxed()</code> instead.</p></div>
<details class="viet-answer"><summary>🇻🇳 Đáp án (Tiếng Việt)</summary><p>Hầu hết các phép chuyển đổi thuộc ba nhóm. <strong>Chuyển bất cứ thứ gì sang String</strong> dùng <code>String.valueOf(x)</code> — nó an toàn với null đối với object, khác với <code>x.toString()</code>. <strong>Chuyển String sang kiểu nguyên thủy</strong> dùng method parse của lớp wrapper: <code>Integer.parseInt</code>, <code>Double.parseDouble</code>, <code>Float.parseFloat</code>, tất cả đều ném <code>NumberFormatException</code> khi dữ liệu không hợp lệ. <strong>Chuyển giữa mảng và List</strong> dùng <code>Arrays.asList(arr)</code> và <code>list.toArray(new String[0])</code>. Chỗ duy nhất phá vỡ quy tắc này là mảng kiểu nguyên thủy: <code>int[]</code> không phải là <code>Integer[]</code>, nên <code>Arrays.asList(intArray)</code> trả về một list chỉ có đúng một phần tử chứa chính mảng đó. Hãy dùng stream với <code>boxed()</code> thay thế.</p></details>
<table>
<tr><th>Conversion</th><th>Java</th></tr>
<tr><td><code>int → String</code></td><td><code>String.valueOf(x)</code></td></tr>
<tr><td><code>String → int</code></td><td><code>Integer.parseInt(s)</code></td></tr>
<tr><td><code>float → String</code></td><td><code>String.valueOf(x)</code></td></tr>
<tr><td><code>String → float</code></td><td><code>Float.parseFloat(s)</code></td></tr>
<tr><td><code>double → String</code></td><td><code>String.valueOf(x)</code></td></tr>
<tr><td><code>String → double</code></td><td><code>Double.parseDouble(s)</code></td></tr>
<tr><td><code>String → char[]</code></td><td><code>s.toCharArray()</code></td></tr>
<tr><td><code>char[] → String</code></td><td><code>new String(arr)</code></td></tr>
<tr><td><code>String[] → List</code></td><td><code>Arrays.asList(arr)</code></td></tr>
<tr><td><code>List → String[]</code></td><td><code>list.toArray(new String[0])</code></td></tr>
<tr><td><code>ArrayList → Array</code></td><td><code>list.toArray(new Type[0])</code></td></tr>
<tr><td><code>int[] → List&lt;Integer&gt;</code></td><td>loop through array</td></tr>
<tr><td><code>List&lt;Integer&gt; → int[]</code></td><td>loop through list</td></tr>
<tr><td><code>int[] → String</code></td><td><code>Arrays.toString(arr)</code></td></tr>
</table>
<p><strong>Stream one-liners for the two "loop through" rows:</strong></p>
<pre>// int[] → List&lt;Integer&gt;   (boxed() turns IntStream into Stream&lt;Integer&gt;)
List&lt;Integer&gt; list = Arrays.stream(arr).boxed().toList();          // Java 16+
List&lt;Integer&gt; list = Arrays.stream(arr).boxed()
                           .collect(Collectors.toList());          // Java 8

// List&lt;Integer&gt; → int[]   (mapToInt unboxes; throws NPE on a null element)
int[] arr = list.stream().mapToInt(Integer::intValue).toArray();</pre>
<pre>// The traps behind the table:

Arrays.asList(intArray);     // List&lt;int[]&gt; of size 1 — NOT List&lt;Integer&gt;!
Arrays.asList("a", "b");     // fixed-size VIEW — add()/remove() throw
                             //   UnsupportedOperationException; set() writes through
new ArrayList&lt;&gt;(Arrays.asList(arr));   // mutable copy
List.of(arr);                          // Java 9+, fully immutable, rejects nulls

list.toArray(new String[0]);  // idiomatic — 0 is FASTER than list.size() on modern JVMs
list.toArray();               // returns Object[], not String[]

String.valueOf(chars);        // "abc"  — char[] has a dedicated overload
String.valueOf(intArr);       // "[I@1b6d" — no overload, falls back to Object.toString()
Arrays.toString(intArr);      // "[1, 2, 3]"        — what you actually want
Arrays.deepToString(matrix);  // for nested arrays

String.valueOf(objOrNull);    // "null"  — null-safe
objOrNull.toString();         // NullPointerException
Integer.parseInt("12a");      // NumberFormatException (unchecked — validate input!)
Integer.parseInt(null);       // NumberFormatException, not NPE</pre>
<div class="key-point"><code>parseInt</code> returns the primitive <code>int</code>; <code>Integer.valueOf</code> returns the boxed <code>Integer</code> and caches −128..127, which is why <code>==</code> on boxed values is unreliable. Prefer <code>parseXxx</code> when you need a primitive to avoid a pointless box, and <code>String.valueOf</code> over <code>"" + x</code> (which compiles to a <code>StringBuilder</code> or <code>invokedynamic</code> concat).</div>`,
      },

      // ──── 2. OOP & CLASS DESIGN — inheritance, equality, immutability, patterns ────
      {
...
<details class="viet-answer"><summary>🇻🇳 Đáp án (Tiếng Việt)</summary><p>Bốn tính chất cốt lõi là <strong>encapsulation</strong>, <strong>abstraction</strong>, <strong>inheritance</strong> và <strong>polymorphism</strong>. Encapsulation che giấu và bảo vệ dữ liệu của một object. Abstraction ẩn đi chi tiết và chỉ phơi bày những gì một class có thể làm, thường thông qua interface. Inheritance giúp tái sử dụng code, còn polymorphism cho phép một method hoạt động theo nhiều cách khác nhau. Một thói quen tốt là ưu tiên <strong>composition hơn inheritance</strong>, vì inheritance tạo ra sự phụ thuộc chặt (tight coupling).</p></details>
<ul>
...
<details class="viet-answer"><summary>🇻🇳 Đáp án (Tiếng Việt)</summary><p>Một <strong>abstract class</strong> có thể có constructor, field mang trạng thái và mọi mức truy cập, nhưng một class chỉ có thể extends duy nhất một abstract class. Một <strong>interface</strong> là một bản giao ước mà một class có thể implement nhiều cái, và từ Java 8 nó còn có thể có method <code>default</code> và <code>static</code>. Hãy dùng interface để mô tả những gì một class có thể làm. Dùng abstract class khi các class chia sẻ chung trạng thái hoặc một cấu trúc nền tảng chung.</p></details>
<ul>
...
<details class="viet-answer"><summary>🇻🇳 Đáp án (Tiếng Việt)</summary><p>Overloading nghĩa là cùng một tên method nhưng khác tham số, và trình biên dịch chọn cái nào ngay lúc biên dịch (static polymorphism). Overriding nghĩa là một subclass thay thế method của lớp cha với cùng signature, và việc chọn được thực hiện lúc runtime dựa trên kiểu thực sự của đối tượng (dynamic dispatch). Nói ngắn gọn, overloading dựa vào kiểu khai báo, còn overriding dựa vào kiểu lúc runtime. Static method không thể bị override; việc định nghĩa lại một static method được gọi là hiding.</p></details>
<ul>
...
<details class="viet-answer"><summary>🇻🇳 Đáp án (Tiếng Việt)</summary><p>Overload resolution xảy ra lúc <strong>biên dịch</strong> dựa trên kiểu khai báo của các đối số. Compiler thử ba giai đoạn theo thứ tự và chỉ chuyển sang giai đoạn tiếp theo nếu không có cái nào khớp: đầu tiên là khớp chính xác và <em>widening</em> kiểu nguyên thủy (int sang long), rồi tới <em>boxing và unboxing</em>, và cuối cùng là <em>varargs</em>. Nên một <code>int</code> thuần sẽ chọn method nhận <code>long</code> hơn method nhận <code>Integer</code>, và hơn cả method nhận <code>Object...</code>. Một <code>null</code> trần khớp với mọi kiểu tham chiếu và chọn kiểu <strong>cụ thể nhất</strong>; nếu hai kiểu tham chiếu không liên quan nhau thì đó là lỗi biên dịch mơ hồ (ambiguous), mà bạn khắc phục bằng một phép cast. Các overload chỉ khác nhau ở những điểm nhỏ này rất khó đọc, nên hãy đổi tên hoặc cast thay vì làm vậy.</p></details>
...
<details class="viet-answer"><summary>🇻🇳 Đáp án (Tiếng Việt)</summary><p>Không, bạn không thể override một static method; bạn <strong>che giấu (hide)</strong> nó. Khác biệt nằm ở chỗ kiểu nào quyết định. Việc override instance method được giải quyết lúc chạy dựa trên kiểu thực của đối tượng, nhưng một static method được giải quyết lúc biên dịch dựa trên kiểu <em>khai báo</em> của tham chiếu. Nên với <code>Parent p = new Child()</code>, <code>p.staticMethod()</code> gọi phiên bản của Parent bất kể đối tượng là gì, trong khi một instance method được override sẽ gọi phiên bản của Child. Đó là lý do vì sao gọi một static method thông qua một instance dễ gây nhầm lẫn, và static method nên được gọi qua tên class.</p></details>
...
<details class="viet-answer"><summary>🇻🇳 Đáp án (Tiếng Việt)</summary><p><strong>Vấn đề kim cương (diamond problem)</strong> xảy ra khi một lớp kế thừa cùng một default method từ hai interface không liên quan; code sẽ không biên dịch được cho đến khi bạn xử lý xung đột. Các quy tắc là: phương thức của một lớp cụ thể thắng mọi default của interface, một sub-interface cụ thể hơn thắng interface cha của nó, và nếu thực sự hòa thì bạn phải override phương thức. Bên trong phần override, bạn có thể gọi một cái cụ thể bằng <code>Interface.super.method()</code>. Java tránh được vấn đề kim cương kinh điển nhờ chỉ cho phép kế thừa đơn ở cấp lớp.</p></details>
...
<details class="viet-answer"><summary>🇻🇳 Đáp án (Tiếng Việt)</summary><p><code>==</code> so sánh reference đối với object (chúng có phải là cùng một object trong bộ nhớ hay không) và so sánh giá trị đối với kiểu nguyên thủy. <code>equals()</code> so sánh nội dung thực sự và có thể được override trong class của bạn. Khi bạn override <code>equals()</code>, bạn bắt buộc phải override cả <code>hashCode()</code>, nếu không <code>HashMap</code> và <code>HashSet</code> sẽ hoạt động sai. Luôn dùng <code>.equals()</code> cho các kiểu wrapper như <code>Integer</code>, vì Java cache lại các giá trị nhỏ từ -128 đến 127.</p></details>
<ul>
...
<details class="viet-answer"><summary>🇻🇳 Đáp án (Tiếng Việt)</summary><p><code>127 == 127</code> là true nhưng <code>128 == 128</code> là false vì autoboxing và Integer cache. Viết <code>Integer a = 127</code> sẽ gọi <code>Integer.valueOf(127)</code>, vốn cache các giá trị từ -128 đến 127, nên cả hai biến trỏ tới cùng một đối tượng và <code>==</code> là true. Tới 128 thì giá trị nằm ngoài cache, nên hai đối tượng riêng biệt được tạo ra và <code>==</code> so sánh tham chiếu, cho ra false. Quy tắc an toàn: đừng bao giờ dùng <code>==</code> với kiểu boxed, luôn dùng <code>.equals()</code>.</p></details>
...
<details class="viet-answer"><summary>🇻🇳 Đáp án (Tiếng Việt)</summary><p>Quy tắc là: nếu hai đối tượng bằng nhau thì chúng phải trả về cùng một <code>hashCode</code>. Nếu bạn phá vỡ điều này, các collection dựa trên hash sẽ hỏng một cách âm thầm. Bạn có thể <code>put</code> một key vào <code>HashMap</code> rồi sau đó không <code>get</code> lại được, vì nó tìm sai bucket. Tệ hơn là với một <strong>key có thể thay đổi (mutable)</strong>: nếu bạn đổi một field dùng trong <code>hashCode</code> sau khi đã thêm vào map, entry đó trở nên mất tích. Bạn không tìm thấy cũng không xóa được nó, nên nó nằm lì trong bộ nhớ. Đó là lý do key của map nên bất biến, chẳng hạn record hoặc value object.</p></details>
...
<details class="viet-answer"><summary>🇻🇳 Đáp án (Tiếng Việt)</summary><p>Để làm cho một lớp trở nên <strong>bất biến (immutable)</strong>: đánh dấu lớp là <code>final</code>, để tất cả các trường là <code>private final</code>, không thêm setter, và gán mọi trường ngay trong constructor. Đồng thời tạo <strong>bản sao phòng vệ (defensive copy)</strong> cho mọi đầu vào có thể thay đổi, và trả về bản sao hoặc view không thể sửa đổi, để phía gọi không thể thay đổi trạng thái nội bộ. Những đối tượng như vậy an toàn với đa luồng và có thể dùng làm key của map. Với các lớp chứa dữ liệu đơn giản, một <code>record</code> sẽ tự động mang lại hầu hết những điều này.</p></details>
...
<details class="viet-answer"><summary>🇻🇳 Đáp án (Tiếng Việt)</summary><p><strong>Shallow copy</strong> sao chép đối tượng bên ngoài nhưng dùng chung các đối tượng bên trong, nên thay đổi một đối tượng bên trong sẽ ảnh hưởng đến cả hai bản sao. <strong>Deep copy</strong> sao chép luôn cả các đối tượng bên trong, nên hai bản sao hoàn toàn độc lập. Hãy tránh <code>clone()</code> và Cloneable; thay vào đó dùng copy constructor, static factory, hoặc serialization. Làm cho các đối tượng bên trong trở nên <strong>bất biến</strong> là lựa chọn đơn giản và an toàn nhất.</p></details>
<ul>
...
<details class="viet-answer"><summary>🇻🇳 Đáp án (Tiếng Việt)</summary><p><strong>record</strong> là cách viết ngắn gọn cho một lớp dữ liệu bất biến (immutable). Chỉ một dòng là bạn đã có các trường, constructor, các phương thức truy cập, cùng với <code>equals()</code>, <code>hashCode()</code> và <code>toString()</code> dựa trên giá trị. Record rất phù hợp cho DTO, value object và response của API. Chúng là final và các trường không thể thay đổi, nên đừng dùng khi bạn cần dữ liệu có thể thay đổi hoặc cần kế thừa.</p></details>
...
<details class="viet-answer"><summary>🇻🇳 Đáp án (Tiếng Việt)</summary><p><strong>sealed class</strong> cho phép bạn liệt kê chính xác những lớp nào được phép kế thừa nó, thông qua mệnh đề <code>permits</code>. Mỗi lớp con được phép phải được đánh dấu là <code>final</code>, <code>sealed</code> hoặc <code>non-sealed</code>. Vì đã biết trọn bộ các kiểu con, một câu lệnh <code>switch</code> kết hợp pattern matching có thể được kiểm tra là <strong>bao phủ đầy đủ (exhaustive)</strong>, nên không cần nhánh default. Kết hợp cùng record và switch pattern, đây là cách an toàn để mô hình hóa một tập kiểu đóng.</p></details>
...
<details class="viet-answer"><summary>🇻🇳 Đáp án (Tiếng Việt)</summary><p>Một <code>enum</code> trong Java là một class thực sự, không chỉ là một con số được đặt tên. Mỗi constant là một instance cố định duy nhất, nên nó có thể chứa field, định nghĩa method và implement interface. Điều này mở ra các pattern hữu ích: mỗi constant có thể cung cấp hành vi riêng (strategy), mô hình hóa một state machine an toàn về kiểu, hoặc đóng vai một singleton gọn gàng mà JVM giữ ở đúng một instance và vẫn an toàn khi serialize. Một enum không thể extend một class khác vì nó đã extend <code>Enum</code>, nhưng nó có thể implement các interface.</p></details>
...
<details class="viet-answer"><summary>🇻🇳 Đáp án (Tiếng Việt)</summary><p><code>new</code> chạy theo một thứ tự cố định. Đầu tiên class được load và code static của nó chạy một lần. Sau đó bộ nhớ được cấp phát, và tất cả các field bắt đầu với giá trị mặc định như <code>0</code> hoặc <code>null</code>. Tiếp theo constructor của lớp cha chạy trước, rồi đến các giá trị field và init block theo thứ tự trong source, rồi thân constructor, và cuối cùng reference của object được trả về. Hai cái bẫy thường gặp: các field vẫn giữ giá trị mặc định trước khi code khởi tạo chạy, và phần thiết lập static chỉ chạy một lần chứ không phải cho mỗi object.</p></details>
...
<details class="viet-answer"><summary>🇻🇳 Đáp án (Tiếng Việt)</summary><p>Có hai quy tắc cần nhớ. Thứ nhất, phần khởi tạo static chạy một lần khi class được load lần đầu, cha trước con, theo thứ tự trong mã nguồn. Thứ hai, với mỗi đối tượng mới, superclass được dựng trước, và bên trong một class thì các instance block cùng phần khởi tạo field chạy theo thứ tự mã nguồn trước phần thân constructor. Vậy nên <code>new Child()</code> chạy: static của cha, static của con (chỉ một lần), rồi instance block và constructor của cha, rồi instance block và constructor của con. Một cái bẫy hay gặp là gọi một <strong>method có thể bị override từ trong constructor</strong>, vì phần override ở subclass chạy trong khi các field của subclass vẫn còn ở giá trị mặc định.</p></details>
<pre>class Parent {
...
<details class="viet-answer"><summary>🇻🇳 Đáp án (Tiếng Việt)</summary><p>SOLID là tập hợp năm nguyên tắc thiết kế giúp giữ cho code dễ thay đổi. Năm nguyên tắc đó là: Single Responsibility, Open/Closed, Liskov Substitution, Interface Segregation và Dependency Inversion. Ý tưởng cốt lõi là <strong>phụ thuộc vào interface</strong> và giao cho mỗi class chỉ một nhiệm vụ. Trong các ứng dụng Spring, các nguyên tắc này rất phù hợp, nhưng bạn không nên lạm dụng chúng mà tạo ra quá nhiều class nhỏ vụn.</p></details>
<ul>
...
<details class="viet-answer"><summary>🇻🇳 Đáp án (Tiếng Việt)</summary><p>Design pattern được chia thành ba nhóm: creational, structural và behavioral. Điều quan trọng nhất là hiểu mỗi pattern giải quyết vấn đề gì và một ví dụ thực tế từ JDK hoặc Spring. Những pattern phổ biến là Builder, Factory, Singleton (nên làm bằng enum), Strategy, Observer, Decorator (dùng trong các stream của java.io) và Proxy (cách Spring AOP và transaction hoạt động). Một số pattern cũ nay đã được tích hợp sẵn vào ngôn ngữ, chẳng hạn Iterator, hoặc lambda thay thế cho Strategy.</p></details>
...
<details class="viet-answer"><summary>🇻🇳 Đáp án (Tiếng Việt)</summary><p>Checked exception kế thừa từ <code>Exception</code>, và compiler buộc bạn phải catch hoặc khai báo chúng (ví dụ <code>IOException</code>). Unchecked exception kế thừa từ <code>RuntimeException</code> và không cần điều này (ví dụ <code>NullPointerException</code>). Error là các sự cố nghiêm trọng của JVM mà bạn không nên catch. Một thói quen hiện đại phổ biến là bọc các checked exception thành unchecked exception tại ranh giới giữa các layer, đồng thời giữ lại nguyên nhân gốc.</p></details>
<ul>
...
<details class="viet-answer"><summary>🇻🇳 Đáp án (Tiếng Việt)</summary><p><code>throw</code> là câu lệnh thực sự phát sinh một đối tượng ngoại lệ tại thời điểm chạy, và nó nằm bên trong thân phương thức. <code>throws</code> được viết trong chữ ký phương thức để khai báo những checked exception nào mà phương thức có thể chuyển tiếp cho phía gọi. Chỉ các checked exception mới bắt buộc phải khai báo bằng <code>throws</code>. Một thực hành tốt là không khai báo quá nhiều ngoại lệ cấp thấp, mà thay vào đó chuyển chúng thành các domain exception rõ nghĩa hơn ở ranh giới hệ thống.</p></details>
<ul>
...
<details class="viet-answer"><summary>🇻🇳 Đáp án (Tiếng Việt)</summary><p><strong>Try-with-resources</strong> tự động đóng bất kỳ tài nguyên nào có triển khai <code>AutoCloseable</code> khi khối lệnh kết thúc, dù kết thúc bình thường hay ném ra ngoại lệ. Các tài nguyên được đóng theo thứ tự ngược lại so với thứ tự mở. Cơ chế này thay thế cho cách dọn dẹp bằng <code>try-finally</code> cũ vốn dễ gây lỗi. Nếu cả phần thân lẫn <code>close()</code> đều ném ngoại lệ, lỗi từ close sẽ được giữ lại dưới dạng ngoại lệ <em>suppressed</em> thay vì che mất ngoại lệ chính.</p></details>
...
<details class="viet-answer"><summary>🇻🇳 Đáp án (Tiếng Việt)</summary><p>Có, và đó chính là lý do bạn không bao giờ nên làm vậy. Một câu <code>return</code> bên trong <code>finally</code> sẽ thay thế bất cứ thứ gì mà <code>try</code> hay <code>catch</code> định trả về, và nó <strong>âm thầm che giấu mọi exception</strong> đang được ném ra, nên bên gọi chỉ nhận về một kết quả bình thường. Một câu <code>throw</code> trong <code>finally</code> cũng gây ra điều tương tự. Việc thay đổi một biến cục bộ trong <code>finally</code> không làm đổi giá trị trả về đã được tính sẵn, nhưng một câu <code>return</code> thì có thay thế nó. Quy tắc: chỉ dùng <code>finally</code> để dọn dẹp, không có <code>return</code> hay <code>throw</code>, và để try-with-resources tự đóng resource giúp bạn.</p></details>
...
<details class="viet-answer"><summary>🇻🇳 Đáp án (Tiếng Việt)</summary><p>Framework này bắt đầu từ <code>Iterable</code> và <code>Collection</code>. <code>Collection</code> chia thành <strong>List</strong> (có thứ tự, dùng chỉ số), <strong>Set</strong> (chỉ chứa phần tử duy nhất), và <strong>Queue/Deque</strong> (làm việc ở hai đầu). <code>Map</code> nằm riêng vì nó lưu các cặp key-value, chứ không phải từng phần tử đơn lẻ. Kỹ năng chính là chọn đúng class cho nhu cầu của bạn: <code>ArrayList</code> khi cần thứ tự, <code>HashSet</code> khi cần phần tử duy nhất, <code>TreeMap</code> khi cần sắp xếp, và <code>HashMap</code> cho dữ liệu key-value.</p></details>
...
<details class="viet-answer"><summary>🇻🇳 Đáp án (Tiếng Việt)</summary><p><code>ArrayList</code> dùng một mảng bên trong. Đọc theo chỉ số rất nhanh (<strong>O(1)</strong>), nhưng thêm hoặc xóa ở giữa thì chậm vì các phần tử khác phải dịch chuyển. <code>LinkedList</code> dùng các node liên kết với nhau. Thêm hoặc xóa thì nhanh nếu bạn đã có sẵn node, nhưng tìm tới một phần tử thì chậm (<strong>O(n)</strong>). Trong hầu hết code thực tế thì <code>ArrayList</code> là lựa chọn tốt hơn vì nó nhanh hơn trên thực tế.</p></details>
...
<details class="viet-answer"><summary>🇻🇳 Đáp án (Tiếng Việt)</summary><p>Một <code>HashMap</code> dùng hash code của key để chọn ra một bucket nơi entry được lưu. Khi hai key rơi vào cùng một bucket (collision), các entry được liên kết với nhau trong bucket đó. Kể từ Java 8, nếu một bucket vượt quá 8 entry và bảng có ít nhất 64 slot, bucket đó sẽ chuyển thành một cây cân bằng, nhờ vậy việc tra cứu vẫn nhanh ở mức O(log n). Map sẽ tăng gấp đôi kích thước khi đầy khoảng 75%. Key nên là immutable, và bạn phải override <code>hashCode()</code> cùng với <code>equals()</code>.</p></details>
<ol>
...
<details class="viet-answer"><summary>🇻🇳 Đáp án (Tiếng Việt)</summary><p>Cả bốn đều implement <code>Map</code> nhưng khác nhau về thứ tự, tốc độ và tính thread-safe. <strong>HashMap</strong> là lựa chọn mặc định: tốc độ O(1) và không có thứ tự. <strong>LinkedHashMap</strong> giữ thứ tự chèn hoặc thứ tự truy cập và là nền tảng tốt cho một LRU cache. <strong>TreeMap</strong> giữ key được sắp xếp với tốc độ O(log n) và hỗ trợ truy vấn theo khoảng và tìm key gần nhất. <strong>ConcurrentHashMap</strong> là lựa chọn thread-safe, không cho phép null key hay null value, và chỉ khóa từng bucket riêng lẻ nên các thao tác ghi vào các bucket khác nhau chạy song song được.</p></details>
...
<details class="viet-answer"><summary>🇻🇳 Đáp án (Tiếng Việt)</summary><p><code>HashMap</code> không an toàn với đa luồng, nên ghi vào nó từ nhiều luồng cùng lúc có thể làm hỏng dữ liệu. <code>ConcurrentHashMap</code> an toàn cho nhiều luồng mà không cần khóa toàn bộ map: nó dùng CAS và khóa theo từng bucket, nên các luồng làm việc trên các bucket khác nhau chạy đồng thời. Điều này khiến nó nhanh hơn nhiều so với <code>Hashtable</code> hay <code>Collections.synchronizedMap</code> vốn khóa mọi thứ. Nó không cho phép key hay value là null, và cung cấp các phương thức nguyên tử như <code>compute</code>, <code>merge</code> và <code>putIfAbsent</code> để cập nhật kết hợp một cách an toàn.</p></details>
<ul>
...
<details class="viet-answer"><summary>🇻🇳 Đáp án (Tiếng Việt)</summary><p><strong>Comparable</strong> mang lại cho một lớp một thứ tự sắp xếp mặc định thông qua phương thức <code>compareTo</code> viết bên trong lớp. <strong>Comparator</strong> là một đối tượng riêng biệt, nên bạn có thể định nghĩa nhiều thứ tự sắp xếp khác nhau bên ngoài lớp. Từ Java 8, bạn có thể dựng comparator dễ dàng bằng <code>Comparator.comparing(...).thenComparing(...)</code>. Hãy dùng <code>Integer.compare</code> thay vì phép trừ, vì phép trừ có thể bị tràn số.</p></details>
<ul>
...
<details class="viet-answer"><summary>🇻🇳 Đáp án (Tiếng Việt)</summary><p>Các iterator kiểu fail-fast (như trên <code>ArrayList</code> hay <code>HashMap</code>) sẽ ném <code>ConcurrentModificationException</code> nếu collection bị thay đổi trong lúc lặp. Các iterator kiểu fail-safe (như trên <code>CopyOnWriteArrayList</code> hay <code>ConcurrentHashMap</code>) làm việc trên một bản sao, nên không bao giờ ném lỗi nhưng có thể bỏ sót những thay đổi mới nhất. Để xóa phần tử trong lúc lặp, hãy dùng <code>remove()</code> của chính iterator hoặc <code>removeIf()</code>, chứ không dùng <code>remove()</code> của collection.</p></details>
<ul>
...
<details class="viet-answer"><summary>🇻🇳 Đáp án (Tiếng Việt)</summary><p>Generics cho phép bạn viết một class hay method dùng được với nhiều kiểu, trong khi trình biên dịch kiểm tra rằng các kiểu là đúng. Điều này mang lại <strong>type safety</strong> và loại bỏ nhu cầu ép kiểu, nên một kiểu sai sẽ bị bắt lúc biên dịch thay vì gây ra <code>ClassCastException</code> lúc runtime. Bạn cũng có thể giới hạn kiểu bằng bound như <code>&lt;T extends Number&gt;</code>. Lưu ý rằng thông tin kiểu generic bị xóa lúc runtime (type erasure), nên <code>new T()</code> và <code>instanceof T</code> là không được phép.</p></details>
...
<details class="viet-answer"><summary>🇻🇳 Đáp án (Tiếng Việt)</summary><p>Type erasure nghĩa là generic chỉ tồn tại lúc biên dịch. Compiler kiểm tra kiểu và chèn thêm các phép ép kiểu (cast), rồi loại bỏ các type parameter, nên lúc chạy chỉ còn lại một <code>List</code> thô (raw). Điều này được làm để giữ tương thích với code Java cũ. Đó là lý do bạn không thể overload <code>List&lt;String&gt;</code> với <code>List&lt;Integer&gt;</code>: sau erasure cả hai method có cùng chữ ký <code>List</code>, nên code sẽ không biên dịch được. Cũng vì lý do đó mà bạn không thể làm <code>new T()</code>, không thể dùng <code>instanceof List&lt;String&gt;</code>, hay tạo một mảng <code>List&lt;String&gt;[]</code>.</p></details>
...
<details class="viet-answer"><summary>🇻🇳 Đáp án (Tiếng Việt)</summary><p>Một functional interface có đúng một abstract method. Một lambda là cách viết ngắn gọn cho method duy nhất đó, và trình biên dịch tự suy ra nó khớp với interface nào dựa trên ngữ cảnh. Java cung cấp sẵn nhiều interface trong <code>java.util.function</code> như <code>Predicate</code>, <code>Function</code>, <code>Consumer</code> và <code>Supplier</code>. Khác với anonymous class, trong một lambda thì <code>this</code> trỏ tới đối tượng bên ngoài, các biến được capture phải là effectively final, và không có file <code>.class</code> phụ nào được tạo ra.</p></details>
...
<details class="viet-answer"><summary>🇻🇳 Đáp án (Tiếng Việt)</summary><p>Các thao tác <strong>intermediate</strong> như <code>map()</code> và <code>filter()</code> trả về một stream mới và có tính lazy, nên chúng chưa chạy ngay. Một thao tác <strong>terminal</strong> như <code>collect()</code> hay <code>findFirst()</code> mới khởi động công việc thực sự và tạo ra kết quả. Vì stream là lazy, cả chuỗi chạy trong một lượt duy nhất và có thể dừng sớm ngay khi tìm được kết quả. Một stream chỉ dùng được một lần; sau một thao tác terminal, dùng lại nó sẽ ném ra <code>IllegalStateException</code>.</p></details>
...
<details class="viet-answer"><summary>🇻🇳 Đáp án (Tiếng Việt)</summary><p>Bốn phương thức này tạo nên một pipeline xử lý dữ liệu. <code>filter</code> chỉ giữ lại những phần tử thỏa điều kiện, <code>map</code> biến đổi mỗi phần tử thành thứ khác, <code>reduce</code> gộp tất cả phần tử thành một giá trị duy nhất, còn <code>collect</code> thu kết quả vào một container như list hay map. Các thao tác như <code>map</code> và <code>filter</code> có tính <strong>lazy</strong> và chỉ thực sự chạy khi một bước kết thúc như <code>collect</code> hoặc <code>reduce</code> được gọi. Để nhóm và dựng map, <code>Collectors.groupingBy</code> và <code>toMap</code> rất hữu ích.</p></details>
...
<details class="viet-answer"><summary>🇻🇳 Đáp án (Tiếng Việt)</summary><p><code>Optional</code> là một container hoặc chứa một giá trị hoặc rỗng. Nó làm cho việc một giá trị có thể vắng mặt trở nên rõ ràng ngay trong signature của method, buộc người gọi phải xử lý trường hợp rỗng thay vì gặp phải <code>NullPointerException</code>. Hãy dùng nó chủ yếu như một <strong>kiểu trả về</strong>, chứ không dùng cho field, tham số hay phần tử của collection. Đọc nó bằng các method như <code>map()</code>, <code>orElseGet()</code> hoặc <code>orElseThrow()</code>, và không bao giờ trả về <code>null</code> từ một method có kiểu trả về là <code>Optional</code>.</p></details>
...
<details class="viet-answer"><summary>🇻🇳 Đáp án (Tiếng Việt)</summary><p>Tất cả <strong>parallel stream</strong> đều dùng chung một ForkJoinPool duy nhất cho toàn JVM, nên một tác vụ chậm hoặc bị blocking có thể làm đói tài nguyên của mọi parallel stream khác trong ứng dụng. Chúng chỉ có lợi khi dataset lớn, thao tác nặng về CPU, và nguồn dữ liệu chia đều được như array hoặc ArrayList. Các thao tác không được có shared mutable state, nếu không kết quả sẽ sai. Với công việc I/O, hãy dùng executor hoặc virtual thread thay thế, và luôn đo lường trước khi cho rằng parallel sẽ nhanh hơn.</p></details>
...
<div class="key-point">The common ForkJoinPool has <code>Runtime.availableProcessors() - 1</code> threads. One slow parallel stream can starve the entire app. Default to sequential streams — parallelize only after profiling shows a bottleneck.</div>`,
      ...
      // ──── 6. JVM INTERNALS — memory, GC, class loading, reflection ────
      {
...
<details class="viet-answer"><summary>🇻🇳 Đáp án (Tiếng Việt)</summary><p><strong>Stack</strong> lưu các lời gọi method, biến cục bộ và reference, mỗi thread có stack riêng của mình. <strong>Heap</strong> lưu các object, được chia sẻ bởi tất cả các thread và được dọn dẹp bởi garbage collector. Thông tin về class được giữ trong <strong>Metaspace</strong> kể từ Java 8. Với code đa luồng, ý tưởng then chốt là <strong>happens-before</strong>: thay đổi của một thread chỉ được đảm bảo là thread khác nhìn thấy khi bạn dùng các cơ chế như <code>volatile</code>, lock, hoặc thread start và join. Không có nó, CPU và compiler có thể sắp xếp lại thứ tự code và một thread có thể đọc phải giá trị cũ.</p></details>
<ul>
...
<details class="viet-answer"><summary>🇻🇳 Đáp án (Tiếng Việt)</summary><p>Java có nhiều garbage collector, mỗi loại có một mục tiêu khác nhau. <strong>Serial</strong> dùng một thread và phù hợp với ứng dụng rất nhỏ. <strong>Parallel</strong> dùng nhiều thread để đạt throughput cao và là mặc định của Java 8. <strong>G1</strong> chia heap thành các vùng (region) và hướng tới pause ngắn; đây là mặc định hiện đại. <strong>ZGC</strong> và <strong>Shenandoah</strong> là các collector chạy đồng thời, thiết kế cho pause cực ngắn trên heap rất lớn. G1 là lựa chọn mặc định an toàn cho hầu hết hệ thống production.</p></details>
<ul>
...
<details class="viet-answer"><summary>🇻🇳 Đáp án (Tiếng Việt)</summary><p>Các loại reference này kiểm soát mức độ garbage collector cố giữ một object. Một <strong>soft reference</strong> được giữ lại cho đến khi JVM sắp cạn bộ nhớ, phù hợp với cache nhạy cảm về bộ nhớ. Một <strong>weak reference</strong> bị thu hồi ngay ở lần GC kế tiếp khi không còn strong reference nào trỏ tới, đây là cách <code>WeakHashMap</code> hoạt động. Một <strong>phantom reference</strong> không thể đọc được và chỉ báo hiệu rằng object đã sẵn sàng để dọn dẹp, khiến nó là lựa chọn an toàn thay cho finalizer. Trong thực tế, một thư viện cache như Caffeine thường lo phần này giúp bạn.</p></details>
...
<details class="viet-answer"><summary>🇻🇳 Đáp án (Tiếng Việt)</summary><p>Trong Java, <strong>memory leak</strong> nghĩa là những object vẫn còn được tham chiếu tới nhưng không còn cần đến nữa, nên garbage collector không thể giải phóng chúng. Nguyên nhân phổ biến là các static collection ngày càng phình to, listener không bao giờ được gỡ, ThreadLocal không được xóa trong pooled thread, resource không được đóng, và classloader leak khi redeploy. Một dấu hiệu cảnh báo là bộ nhớ heap cứ tăng dần và không bao giờ giảm ngay cả sau một lần full GC. Để tìm nguyên nhân, hãy tạo heap dump bằng <code>jmap</code> và phân tích nó trong một công cụ như Eclipse MAT.</p></details>
...
<details class="viet-answer"><summary>🇻🇳 Đáp án (Tiếng Việt)</summary><p>Java có ba class loader chính: <strong>Bootstrap</strong> cho các lớp lõi của JDK, <strong>Platform</strong>, và <strong>Application</strong> cho classpath của riêng bạn. Chúng hoạt động theo cơ chế <strong>parent delegation</strong>: một loader sẽ nhờ loader cha nạp lớp trước khi tự mình thử. Điều này mang lại tính bảo mật (bạn không thể thay thế các lớp lõi như <code>String</code>) và định danh kiểu chính xác (một lớp được nhận diện bằng tên của nó cộng với loader đã định nghĩa nó). Một số công cụ như JDBC và SPI cố tình phá vỡ quy tắc này bằng cách dùng thread context class loader.</p></details>
...
<details class="viet-answer"><summary>🇻🇳 Đáp án (Tiếng Việt)</summary><p><strong>Reflection</strong> cho phép code xem xét và thay đổi các lớp, phương thức và trường trong khi chương trình đang chạy, kể cả những thành phần private. Nó được các framework như Spring, Hibernate, Jackson và JUnit sử dụng. Tốt nhất nên giữ reflection bên trong các framework và không dùng trong code nghiệp vụ thông thường, bởi nó chậm, phá vỡ tính đóng gói và không có kiểm tra tại thời điểm biên dịch. Với nhu cầu động của riêng bạn, một interface thường kết hợp với đa hình thường là lựa chọn tốt hơn.</p></details>
...
<details class="viet-answer"><summary>🇻🇳 Đáp án (Tiếng Việt)</summary><p>Cả hai đều tạo ra các proxy object lúc runtime để thêm hành vi bổ sung như transaction hoặc logging. Một <strong>JDK dynamic proxy</strong> yêu cầu class đích phải implement một <strong>interface</strong>, và nó proxy chính interface đó. Một <strong>CGLIB proxy</strong> tạo ra một <strong>subclass</strong> của class đích, nên nó hoạt động được với cả class không có interface, nhưng không thể proxy các final class hoặc final method. Spring dùng JDK proxy khi có interface và dùng CGLIB trong các trường hợp còn lại. Một hệ quả quan trọng: khi gọi một method trên <code>this</code> bên trong cùng một bean thì sẽ bỏ qua proxy, nên <code>@Transactional</code> hay <code>@Cacheable</code> trên lời gọi đó sẽ không có tác dụng gì.</p></details>
...
<details class="viet-answer"><summary>🇻🇳 Đáp án (Tiếng Việt)</summary><p>Một annotation tùy chỉnh là metadata được khai báo bằng <code>@interface</code>. Hai meta-annotation quan trọng nhất: <code>@Target</code> xác định nơi nó có thể được dùng, và <code>@Retention</code> xác định nó được giữ lại trong bao lâu. Dùng retention <code>SOURCE</code> cho các marker chỉ tồn tại lúc compile và <code>RUNTIME</code> khi nó cần được đọc bằng reflection, như Spring và các trình validator vẫn làm. Có hai cách xử lý annotation: reflection lúc runtime, linh hoạt nhưng có chi phí, và các annotation processor lúc compile sinh ra code mà không có chi phí runtime, đây là cách Lombok, MapStruct và Dagger hoạt động.</p></details>
...
<details class="viet-answer"><summary>🇻🇳 Đáp án (Tiếng Việt)</summary><p>Việc troubleshoot được thực hiện bằng cách khớp công cụ với triệu chứng và thu thập bằng chứng trước tiên. Với <strong>CPU cao</strong>, hãy tìm thread đang bận bằng <code>top -H</code>, đổi id của nó sang hệ hex, rồi khớp trong một thread dump lấy bằng <code>jstack</code>; thường là do vòng lặp chặt hoặc garbage collection nặng. Với <strong>OutOfMemoryError</strong>, chạy với cờ <code>-XX:+HeapDumpOnOutOfMemoryError</code> và mở heap dump trong Eclipse MAT để xem cái gì đang giữ bộ nhớ. Với một tình huống <strong>treo (hang)</strong>, hãy lấy hai hoặc ba thread dump cách nhau vài giây: nhiều thread bị chặn trên khóa của nhau nghĩa là deadlock, còn nhiều thread trên cùng một khóa nghĩa là lock contention. Luôn thu thập thread dump, heap dump và GC log trước khi đoán.</p></details>
...
<div class="key-point">The senior signal is a <strong>workflow</strong>, not tool names: reproduce → capture (thread/heap dump, GC logs <code>-Xlog:gc*</code>) → analyze → fix → verify with the same measurement. Guessing flags without evidence is the anti-pattern.</div>`,
      ...
      {
        q: 'Concurrency cheat sheet: threads, synchronization, executors, futures, concurrent collections, locks, atomics, synchronizers',
        difficulty: 'medium',
        a: `<div class="interview-answer"><p>The whole concurrency toolbox is eight layers. <strong>Threads</strong> define the work (<code>Runnable</code>, <code>Callable</code>), <strong>synchronization</strong> protects shared data (<code>synchronized</code>, <code>volatile</code>, <code>wait</code>/<code>notify</code>), <strong>executors</strong> reuse threads instead of creating them, <strong>CompletableFuture</strong> composes async steps, <strong>concurrent collections</strong> give you thread-safe data structures, <strong>locks</strong> add tryLock/timeout/fairness on top of <code>synchronized</code>, <strong>atomics</strong> give lock-free counters via CAS, and <strong>synchronizers</strong> coordinate groups of threads. The two lines to remember: <code>synchronized</code> = mutual exclusion + visibility, <code>volatile</code> = visibility only; and <code>start()</code> creates a thread while <code>run()</code> does not.</p></div>
<details class="viet-answer"><summary>🇻🇳 Đáp án (Tiếng Việt)</summary><p>Toàn bộ bộ công cụ concurrency gồm tám tầng. <strong>Thread</strong> định nghĩa công việc (<code>Runnable</code>, <code>Callable</code>), <strong>synchronization</strong> bảo vệ dữ liệu dùng chung (<code>synchronized</code>, <code>volatile</code>, <code>wait</code>/<code>notify</code>), <strong>executor</strong> tái sử dụng thread thay vì tạo mới, <strong>CompletableFuture</strong> kết hợp các bước bất đồng bộ, <strong>concurrent collection</strong> cung cấp cấu trúc dữ liệu thread-safe, <strong>lock</strong> bổ sung tryLock/timeout/fairness so với <code>synchronized</code>, <strong>atomic</strong> cho phép đếm lock-free bằng CAS, còn <strong>synchronizer</strong> điều phối nhiều thread với nhau. Hai câu cần nhớ: <code>synchronized</code> = loại trừ lẫn nhau + visibility, <code>volatile</code> = chỉ visibility; và <code>start()</code> tạo thread mới còn <code>run()</code> thì không.</p></details>
<p><strong>1. Thread basics</strong></p>
<table>
<tr><th>Concept</th><th>Remember</th></tr>
<tr><td><code>Thread</code></td><td>Extend <code>Thread</code> — burns the single inheritance slot, avoid.</td></tr>
<tr><td><code>Runnable</code></td><td>Preferred for tasks; the class can still extend something else.</td></tr>
<tr><td><code>Callable&lt;V&gt;</code></td><td>Like <code>Runnable</code> + return value + checked exception.</td></tr>
<tr><td>Lifecycle</td><td>NEW → RUNNABLE → BLOCKED / WAITING / TIMED_WAITING → TERMINATED</td></tr>
<tr><td><code>start()</code></td><td>Creates a new thread.</td></tr>
<tr><td><code>run()</code></td><td>Executes the task; calling it directly does <strong>not</strong> create a thread.</td></tr>
<tr><td><code>join()</code></td><td>Wait for another thread to finish.</td></tr>
<tr><td><code>sleep()</code></td><td>Pause the current thread (keeps held locks).</td></tr>
<tr><td><code>interrupt()</code></td><td>Request interruption — cooperative, not a kill.</td></tr>
</table>
<div class="key-point">⭐ Interview: <code>start()</code> ≠ <code>run()</code> → always use <code>start()</code> to create a new thread.</div>
<p><strong>2. Synchronization</strong> — shared data + multiple threads → race condition.</p>
<table>
<tr><th>Tool</th><th>Key idea</th></tr>
<tr><td><code>synchronized</code></td><td>Mutual exclusion + visibility.</td></tr>
<tr><td><code>volatile</code></td><td>Visibility only, no atomicity.</td></tr>
<tr><td><code>wait()</code></td><td>Release the lock and wait.</td></tr>
<tr><td><code>notify()</code></td><td>Wake one waiting thread.</td></tr>
<tr><td><code>notifyAll()</code></td><td>Wake all waiting threads.</td></tr>
</table>
<pre>synchronized = locking + visibility
volatile     = visibility only

volatile int count;
count++;   // ❌ still NOT thread-safe — ++ is read-modify-write, not atomic</pre>
<p><strong>3. ExecutorService / thread pool</strong> — why a pool? Thread creation is expensive; a pool reuses threads and bounds concurrency.</p>
<table>
<tr><th>Executor</th><th>Use</th></tr>
<tr><td><code>newFixedThreadPool(n)</code></td><td>Fixed number of workers.</td></tr>
<tr><td><code>newCachedThreadPool()</code></td><td>Dynamic workers, reuses idle ones.</td></tr>
<tr><td><code>newSingleThreadExecutor()</code></td><td>Sequential execution.</td></tr>
<tr><td><code>newScheduledThreadPool(n)</code></td><td>Delayed / periodic tasks.</td></tr>
<tr><td><code>newVirtualThreadPerTaskExecutor()</code></td><td>Java 21+ lightweight virtual threads.</td></tr>
</table>
<p>Spring <code>@Async</code> executes the method on a <code>TaskExecutor</code> / thread pool.</p>
<p><strong>4. CompletableFuture</strong> — asynchronous programming + composing tasks.</p>
<pre>thenApply     → transform result
thenAccept    → consume result
thenCombine   → combine 2 futures
allOf         → wait for all
anyOf         → first completed
exceptionally → error handling
handle        → result + error handling

// Think: CompletableFuture = async task pipeline</pre>
<p><strong>5. Concurrent collections</strong></p>
<table>
<tr><th>Collection</th><th>Use</th></tr>
<tr><td><code>ConcurrentHashMap</code></td><td>Thread-safe map, high concurrency.</td></tr>
<tr><td><code>CopyOnWriteArrayList</code></td><td>Many reads, few writes.</td></tr>
<tr><td><code>BlockingQueue</code></td><td>Producer–Consumer.</td></tr>
<tr><td><code>ConcurrentLinkedQueue</code></td><td>Lock-free, non-blocking queue.</td></tr>
</table>
<p><strong>6. Locks</strong></p>
<table>
<tr><th>Lock</th><th>Key idea</th></tr>
<tr><td><code>ReentrantLock</code></td><td><code>synchronized</code> + <code>tryLock</code>, timeout, fairness, interruptible.</td></tr>
<tr><td><code>ReadWriteLock</code></td><td>Multiple readers OR one writer.</td></tr>
<tr><td><code>StampedLock</code></td><td>Optimistic reading; useful for read-heavy workloads.</td></tr>
</table>
<div class="key-point">⭐ When <code>ReentrantLock</code> over <code>synchronized</code>? When you need <code>tryLock()</code>, timeout, fairness, interruptible locking, or a <code>Condition</code>.</div>
<p><strong>7. Atomic classes</strong> — <code>AtomicInteger</code>, <code>AtomicLong</code>, <code>AtomicBoolean</code>, <code>AtomicReference</code>. They use CAS (compare-and-swap) for lock-free atomic operations.</p>
<pre>counter.incrementAndGet();   // instead of synchronizing a simple counter</pre>
<p><strong>8. Synchronizers</strong></p>
<table>
<tr><th>Utility</th><th>Think of it as</th></tr>
<tr><td><code>CountDownLatch</code></td><td>Wait for N events — one-time.</td></tr>
<tr><td><code>CyclicBarrier</code></td><td>Everyone waits → continue together — reusable.</td></tr>
<tr><td><code>Semaphore</code></td><td>Allow N threads at once.</td></tr>
<tr><td><code>Phaser</code></td><td>Flexible multi-phase barrier.</td></tr>
</table>
<div class="key-point">Map of the section that follows: define the task (<code>Runnable</code>/<code>Callable</code>) → protect shared state (<code>synchronized</code>/<code>volatile</code>/locks/atomics) → run it on a pool (<code>ExecutorService</code>, virtual threads) → compose results (<code>CompletableFuture</code>) → coordinate threads (latches, barriers, semaphores).</div>`,
      },
      {
...
<details class="viet-answer"><summary>🇻🇳 Đáp án (Tiếng Việt)</summary><p>Một <strong>process</strong> là một chương trình đang chạy với vùng nhớ riêng của nó. Một <strong>thread</strong> là một đơn vị công việc nhỏ hơn chạy bên trong một process. Các thread trong cùng một process chia sẻ bộ nhớ heap nhưng mỗi thread có stack riêng, nên chúng rẻ để tạo và có thể chia sẻ dữ liệu dễ dàng. Nhược điểm là dữ liệu dùng chung có thể gây race condition, nên bạn cần đến synchronization.</p></details>
<ul>
...
<div class="key-point">Always use <strong>ExecutorService / Thread Pool</strong> over raw threads in production. Creating a new thread for each task is expensive. Thread pool reuses threads like a connection pool reuses DB connections.</div>`,
      ...
      {
        q: 'How many ways are there to create and run a thread in Java? (Thread vs Runnable vs Callable — which is recommended?)',
        difficulty: 'medium',
        a: `<div class="interview-answer"><p>There are two <em>low-level</em> ways to define the work — <strong>extend <code>Thread</code></strong> or <strong>implement <code>Runnable</code></strong> — plus <strong><code>Callable</code></strong>, which is like <code>Runnable</code> but returns a value and may throw a checked exception. On top of those sit the ways you actually run them: <code>ExecutorService</code>, <code>CompletableFuture</code>, parallel streams / <code>ForkJoinPool</code>, virtual threads in Java 21, and <code>@Async</code> in Spring. <strong>Never extend <code>Thread</code></strong> — it burns your single inheritance slot and couples the task to the worker. The recommended default is to write the task as a <code>Runnable</code> (fire-and-forget) or <code>Callable</code> (needs a result) and hand it to an <code>ExecutorService</code>; use <code>CompletableFuture</code> when you need to chain or combine async steps.</p></div>
<details class="viet-answer"><summary>🇻🇳 Đáp án (Tiếng Việt)</summary><p>Có hai cách <em>ở mức thấp</em> để định nghĩa công việc — <strong>extends <code>Thread</code></strong> hoặc <strong>implements <code>Runnable</code></strong> — cộng thêm <strong><code>Callable</code></strong>, giống <code>Runnable</code> nhưng trả về giá trị và có thể ném checked exception. Bên trên chúng là các cách bạn thực sự chạy chúng: <code>ExecutorService</code>, <code>CompletableFuture</code>, parallel stream / <code>ForkJoinPool</code>, virtual thread trong Java 21, và <code>@Async</code> trong Spring. <strong>Đừng bao giờ extends <code>Thread</code></strong> — nó dùng mất suất kế thừa đơn duy nhất và làm task bị dính chặt vào worker. Cách được khuyến nghị là viết task dưới dạng <code>Runnable</code> (chạy rồi quên) hoặc <code>Callable</code> (cần kết quả) rồi đưa cho một <code>ExecutorService</code>; dùng <code>CompletableFuture</code> khi bạn cần nối chuỗi hoặc kết hợp nhiều bước bất đồng bộ.</p></details>
<table>
<tr><th>#</th><th>Way</th><th>Returns</th><th>Recommended? / Typical use</th></tr>
<tr><td>1</td><td><code>extends Thread</code></td><td>Nothing</td><td>❌ Avoid — uses up single inheritance, task coupled to thread. Only for quick demos.</td></tr>
<tr><td>2</td><td><code>implements Runnable</code></td><td>Nothing</td><td>✅ Default for fire-and-forget tasks. A lambda, so it composes with any executor.</td></tr>
<tr><td>3</td><td><code>implements Callable&lt;T&gt;</code></td><td><code>T</code> + checked exception</td><td>✅ When the task produces a <strong>result</strong>. Must be submitted to an executor.</td></tr>
<tr><td>4</td><td><code>ExecutorService</code></td><td><code>Future&lt;T&gt;</code></td><td>✅ <strong>The production default.</strong> Pools and reuses threads, bounds concurrency.</td></tr>
<tr><td>5</td><td><code>CompletableFuture</code></td><td><code>CompletableFuture&lt;T&gt;</code></td><td>✅ Chaining/combining async steps without blocking on <code>get()</code>.</td></tr>
<tr><td>6</td><td>Parallel stream / <code>ForkJoinPool</code></td><td>Stream result</td><td>⚠️ CPU-bound data parallelism only. Shares the common pool — never for blocking IO.</td></tr>
<tr><td>7</td><td>Virtual threads (Java 21)</td><td>Depends</td><td>✅ Massive <strong>IO-bound</strong> concurrency — thread-per-request without the cost.</td></tr>
<tr><td>8</td><td><code>@Async</code> (Spring)</td><td><code>void</code> / <code>CompletableFuture</code></td><td>✅ In Spring apps, backed by a configured <code>ThreadPoolTaskExecutor</code>.</td></tr>
</table>
<p><strong>Runnable vs Callable — the core distinction:</strong></p>
<table>
<tr><th>Aspect</th><th>Runnable</th><th>Callable&lt;T&gt;</th></tr>
<tr><td>Method</td><td><code>void run()</code></td><td><code>T call()</code></td></tr>
<tr><td>Returns a value</td><td>❌ No</td><td>✅ Yes</td></tr>
<tr><td>Checked exceptions</td><td>❌ Cannot throw</td><td>✅ <code>throws Exception</code></td></tr>
<tr><td>Since</td><td>Java 1.0</td><td>Java 5</td></tr>
<tr><td>Run with</td><td><code>new Thread(r)</code>, <code>execute()</code>, <code>submit()</code></td><td><code>submit()</code> / <code>invokeAll()</code> only</td></tr>
<tr><td>Failure surfaces as</td><td>Uncaught handler (silently lost via <code>submit</code>)</td><td><code>ExecutionException</code> from <code>future.get()</code></td></tr>
</table>
<pre>// 1. extends Thread — task IS the thread (avoid)
class Worker extends Thread {
    public void run() { doWork(); }
}
new Worker().start();

// 2. Runnable — task is separate from the thread (preferred shape)
Runnable task = () -> doWork();
new Thread(task).start();

// 3. Callable — returns a value, may throw
Callable&lt;String&gt; job = () -> fetchFromDb();   // checked exception allowed

// 4. ExecutorService — the production default
ExecutorService pool = Executors.newFixedThreadPool(4);
pool.execute(task);                        // Runnable, fire-and-forget
Future&lt;String&gt; f = pool.submit(job);       // Callable → Future
String result = f.get();                   // BLOCKS until done
pool.shutdown();

// 5. CompletableFuture — non-blocking composition
CompletableFuture.supplyAsync(() -> fetchUser(id), pool)
    .thenApply(User::name)
    .thenAccept(System.out::println)
    .exceptionally(ex -> { log.error("failed", ex); return null; });

// 6. Parallel stream — CPU-bound only
long count = list.parallelStream().filter(this::isValid).count();

// 7. Virtual threads (Java 21) — millions of blocking IO tasks
try (var vpool = Executors.newVirtualThreadPerTaskExecutor()) {
    urls.forEach(u -> vpool.submit(() -> download(u)));
}   // close() waits for all tasks</pre>
<pre>// The classic interview trap:
Thread t = new Thread(task);
t.run();     // ✗ runs SYNCHRONOUSLY on the CURRENT thread — no new thread
t.start();   // ✓ asks the JVM for a new thread, which then calls run()
t.start();   // calling start() twice → IllegalThreadStateException</pre>
<div class="key-point">Rule of thumb: <strong>no result</strong> → <code>Runnable</code>; <strong>result or checked exception</strong> → <code>Callable</code>; <strong>always run it through an <code>ExecutorService</code></strong>, not <code>new Thread()</code>. Sizing: CPU-bound → pool ≈ number of cores; IO-bound → a larger pool, or virtual threads on Java 21. Note that <code>submit(runnable)</code> swallows exceptions into the <code>Future</code> — if you never call <code>get()</code>, the failure disappears silently; <code>execute(runnable)</code> routes it to the uncaught-exception handler instead.</div>`,
      },
      {
...
<details class="viet-answer"><summary>🇻🇳 Đáp án (Tiếng Việt)</summary><p><code>volatile</code> đảm bảo <strong>tính hiển thị (visibility)</strong> và <strong>thứ tự (ordering)</strong> giữa các thread, nhưng <strong>không đảm bảo tính nguyên tử (atomicity)</strong>. Nó khiến các thao tác đọc và ghi đi qua main memory và thêm các memory barrier, nhờ vậy một lần ghi sẽ được các lần đọc sau đó ở thread khác nhìn thấy. Nó rất hợp cho một cờ trạng thái với một thread ghi và nhiều thread đọc, và cho reference trong một singleton dùng double-checked-locking. Nó không làm cho <code>i++</code> an toàn, vì đó là thao tác read-modify-write; hãy dùng <code>AtomicInteger</code> cho việc đó.</p></details>
<ul>
...
<details class="viet-answer"><summary>🇻🇳 Đáp án (Tiếng Việt)</summary><p>Một <strong>deadlock</strong> xảy ra khi hai hoặc nhiều thread mỗi cái giữ một lock và chờ một lock mà cái kia đang giữ, nên không cái nào tiếp tục được. Nó cần bốn điều kiện xảy ra cùng lúc: mutual exclusion, hold-and-wait, no preemption, và circular wait. Cách khắc phục phổ biến nhất là <strong>giữ thứ tự lấy lock nhất quán</strong>, tức mọi thread đều lấy các lock theo cùng một thứ tự. Bạn cũng có thể dùng <code>tryLock</code> với timeout, hoặc dùng các lớp an toàn như <code>ConcurrentHashMap</code> để tránh phải tự quản lý lock.</p></details>
...
<details class="viet-answer"><summary>🇻🇳 Đáp án (Tiếng Việt)</summary><p>Cả ba đều cung cấp mutual exclusion nhưng cho mức độ kiểm soát khác nhau. <code>synchronized</code> là đơn giản nhất và tự động nhả lock, ngay cả khi có exception, nên nó không thể bị rò rỉ. <code>ReentrantLock</code> bổ sung các tính năng như <code>tryLock()</code> với timeout, chờ có thể ngắt (interruptible), fairness và condition variable, nhưng bạn phải unlock nó trong một khối <code>finally</code>. <code>ReadWriteLock</code> cho phép nhiều thread đọc cùng lúc trong khi thread ghi vẫn độc quyền, rất phù hợp với dữ liệu đọc nhiều ghi ít.</p></details>
...
<details class="viet-answer"><summary>🇻🇳 Đáp án (Tiếng Việt)</summary><p><code>ExecutorService</code> quản lý một pool các thread và chạy các task giúp bạn, nên bạn không phải tự gọi <code>new Thread()</code>. Tái sử dụng thread từ một pool rẻ hơn nhiều so với tạo một thread mới cho mỗi task. Lớp <code>Executors</code> cung cấp sẵn các pool: fixed, cached, single và scheduled. Trong môi trường production, thường tốt hơn là tự tạo một <code>ThreadPoolExecutor</code> trực tiếp với một queue có giới hạn, và luôn shutdown pool khi xong việc.</p></details>
...
<details class="viet-answer"><summary>🇻🇳 Đáp án (Tiếng Việt)</summary><p><code>Future</code> chỉ cho bạn block trên <code>get()</code>; nó không thể chain, kết hợp hay xử lý lỗi. <code>CompletableFuture</code> là một API bất đồng bộ đầy đủ: bạn có thể nối các bước bằng <code>thenApply()</code> và <code>thenCompose()</code>, kết hợp các tác vụ bằng <code>allOf()</code> và <code>anyOf()</code>, và xử lý lỗi bằng <code>exceptionally()</code> hoặc <code>handle()</code>, tất cả mà không cần block. Mặc định nó dùng common pool của <code>ForkJoinPool</code> dùng chung, nên hãy truyền executor riêng của bạn cho các tác vụ I/O. Nếu bạn không bao giờ chờ nó và không thêm handler xử lý lỗi, các lỗi sẽ âm thầm biến mất.</p></details>
<ul>
...
<details class="viet-answer"><summary>🇻🇳 Đáp án (Tiếng Việt)</summary><p>Những công cụ mức cao này được ưa dùng hơn so với wait/notify tự viết và các lock thô. Một số công cụ phổ biến: các class <strong>Atomic</strong> cho các bộ đếm và cờ không cần lock, <strong>ConcurrentHashMap</strong> cho các map dùng chung, framework <strong>Executor</strong> để chạy tác vụ, <code>CountDownLatch</code> và <code>CyclicBarrier</code> để điều phối lúc khởi động hoặc theo từng giai đoạn, <strong>Semaphore</strong> để giới hạn số thread chạy đồng thời, và các blocking queue để dựng pipeline producer-consumer có back-pressure. <strong>CompletableFuture</strong> nằm ở tầng trên để kết hợp các công việc bất đồng bộ. Những công cụ này đúng đắn và được kiểm thử kỹ, nên chỉ cần đến các lock tường minh khi không công cụ nào phù hợp.</p></details>
...
<details class="viet-answer"><summary>🇻🇳 Đáp án (Tiếng Việt)</summary><p><strong>ThreadLocal</strong> cấp cho mỗi luồng một bản sao riêng của một giá trị, điều mà các framework dùng để lưu dữ liệu theo từng request. Nguy hiểm chính nằm ở thread pool: các luồng được tái sử dụng, nên nếu bạn không gọi <code>remove()</code> trong khối <code>finally</code>, dữ liệu cũ sẽ rò rỉ sang request tiếp theo. Điều này có thể gây rò rỉ bộ nhớ lẫn kết quả sai hoặc không an toàn. Nó cũng không hợp với virtual thread; <code>ScopedValue</code> của Java 21 là giải pháp thay thế hiện đại.</p></details>
...
<details class="viet-answer"><summary>🇻🇳 Đáp án (Tiếng Việt)</summary><p><strong>Virtual thread</strong> (Java 21) là các luồng nhẹ được JVM quản lý chứ không phải hệ điều hành. Khi một virtual thread chờ I/O, nó rời khỏi luồng OS thay vì chặn luồng đó, nên bạn có thể chạy hàng triệu virtual thread mà vẫn viết code chặn (blocking) đơn giản. Chúng phù hợp nhất cho công việc nặng về I/O, không phải công việc nặng về CPU. Đừng dùng pool cho chúng (hãy tạo một cái cho mỗi tác vụ), và tránh dùng <code>synchronized</code> trên các đoạn có thể chặn vì nó có thể ghim (pin) luồng OS; hãy dùng <code>ReentrantLock</code> thay thế.</p></details>
...
<details class="viet-answer"><summary>🇻🇳 Đáp án (Tiếng Việt)</summary><p><code>java.io</code> cũ là blocking và dựa trên stream, dùng một thread cho mỗi kết nối, nên nó không mở rộng được tới hàng nghìn socket. <strong>NIO</strong> dùng buffer và channel và là non-blocking: một thread dùng một <strong>Selector</strong> để theo dõi nhiều channel và chỉ xử lý những channel đã sẵn sàng. Vòng lặp sự kiện sẵn-sàng này chính là <strong>Reactor pattern</strong>, nền tảng mà các công cụ như Netty được xây dựng trên đó. Lưu ý rằng virtual thread trong Java 21 cho phép code blocking đơn giản cũng mở rộng tốt, nên độ phức tạp thêm của NIO chỉ cần đến khi nó thực sự đem lại lợi ích.</p></details>
...
<details class="viet-answer"><summary>🇻🇳 Đáp án (Tiếng Việt)</summary><p>Nên tránh dùng cơ chế serialization tích hợp sẵn của Java trong code mới. Nó là một rủi ro bảo mật vì đọc dữ liệu bytes không đáng tin có thể chạy code của kẻ tấn công, nó dễ vỡ vì bất kỳ thay đổi nào ở class cũng có thể phá vỡ định dạng, và nó không hoạt động được với các ngôn ngữ khác. Lựa chọn tốt hơn là một định dạng rõ ràng, dựa trên schema: <strong>JSON</strong> với Jackson cho các service, hoặc <strong>Protobuf</strong> hay <strong>Avro</strong> khi kích thước nhỏ và việc tiến hóa schema quan trọng. Nếu buộc phải dùng serialization gốc, hãy bật deserialization filter để an toàn hơn.</p></details>
...
<details class="viet-answer"><summary>🇻🇳 Đáp án (Tiếng Việt)</summary><p><strong>Switch expression</strong> trả về một giá trị và dùng nhãn dạng mũi tên không bị rớt xuống (fall-through), nhờ đó tránh được lỗi thiếu break. <strong>Pattern matching</strong> kiểm tra kiểu và ép kiểu trong một bước, và trong switch nó còn có thể khớp record lẫn kiểu, cho code gọn hơn. <strong>Text block</strong> là chuỗi nhiều dòng viết bằng ba dấu nháy, giúp JSON và SQL dễ đọc hơn. Cùng nhau, những tính năng này khiến Java hiện đại (17+) ngắn gọn và rõ ràng hơn.</p></details>
...
<details class="viet-answer"><summary>🇻🇳 Đáp án (Tiếng Việt)</summary><p><strong>JPMS</strong> (Project Jigsaw, Java 9) thêm một tầng phía trên package: một module khai báo trong <code>module-info.java</code> nó <code>requires</code> gì và <code>exports</code> những package nào. Điều này mang lại encapsulation mạnh mẽ và cấu hình đáng tin cậy, nên các dependency thiếu hoặc bị chia tách sẽ được phát hiện ngay lúc khởi động thay vì lỗi về sau lúc runtime. Nó cũng giúp bản thân JDK được chia thành các module, nên có thể dựng một runtime nhỏ gọn bằng jlink. Hầu hết ứng dụng doanh nghiệp vẫn chạy trên classpath (unnamed module), nên trong công việc hằng ngày JPMS chủ yếu xuất hiện dưới dạng các cờ <code>--add-opens</code> khi một framework cần reflection sâu vào một module đã đóng.</p></details>
...
<details class="viet-answer"><summary>🇻🇳 Đáp án (Tiếng Việt)</summary><p>Java 17 và 21 là các bản long-term support (LTS) quan trọng nhất với các team. Java 17 mang tới record, sealed class, pattern matching cho <code>instanceof</code> và switch, cùng text block giúp viết SQL và JSON dễ đọc. Java 21 bổ sung <strong>virtual thread</strong>, giúp code có nhiều thao tác nhập/xuất (I/O) trở nên đơn giản và nhanh, cộng thêm record pattern, pattern matching hoàn thiện cho switch, và sequenced collection. Lời khuyên: chuyển lên Java 21 LTS, dùng record và pattern matching rộng rãi, và dùng virtual thread cho các service nặng về I/O.</p></details>
...
<details class="viet-answer"><summary>🇻🇳 Đáp án (Tiếng Việt)</summary><p>Inversion of Control (IoC) nghĩa là Spring container, chứ không phải code của bạn, tạo và kết nối các đối tượng. Dependency Injection (DI) là cách mà nó truyền các đối tượng đó vào. <strong>Constructor injection</strong> là lựa chọn tốt nhất vì các field có thể là <code>final</code>, các dependency bắt buộc luôn có mặt, và lỗi được phát hiện ngay lúc khởi động. Nó cũng dễ test chỉ với một lệnh <code>new</code> đơn giản cùng các mock object. Nên tránh field injection vì nó che giấu các dependency và không thể để final.</p></details>
...
<details class="viet-answer"><summary>🇻🇳 Đáp án (Tiếng Việt)</summary><p>Cả bốn đều là stereotype annotation và là các dạng đặc biệt của <code>@Component</code>, nên component scanning sẽ biến chúng thành bean. Phần lớn khác biệt chỉ là để đánh dấu class thuộc tầng nào. Nhưng có hai cái thêm hành vi thực sự: <code>@Repository</code> chuyển các lỗi database thành các kiểu <code>DataAccessException</code> của Spring, còn <code>@Controller</code> (hoặc <code>@RestController</code>) khiến các method xử lý web request. Nên chọn đúng annotation cụ thể để code rõ ràng hơn và có thêm hành vi đi kèm.</p></details>
...
<details class="viet-answer"><summary>🇻🇳 Đáp án (Tiếng Việt)</summary><p>Mọi request đều đi qua front controller, chính là <strong>DispatcherServlet</strong>. Nó dùng một HandlerMapping để tìm đúng method controller, gọi method đó thông qua một HandlerAdapter, và các argument resolver bind cùng validate request vào các tham số của method. Giá trị trả về được đưa tới một ViewResolver cho MVC, hoặc với REST thì một HttpMessageConverter (Jackson) biến nó thành response body. Bao quanh quy trình này là các Filter ở mức servlet và các HandlerInterceptor ở mức Spring, và bất kỳ lỗi nào cũng được chuyển tới một HandlerExceptionResolver, đây là nơi <code>@ControllerAdvice</code> gắn vào. Nắm được pipeline này cho thấy đúng tầng để đặt các logic dùng chung như auth, logging và tracing.</p></details>
...
<details class="viet-answer"><summary>🇻🇳 Đáp án (Tiếng Việt)</summary><p>Việc xử lý exception nên được tập trung lại thay vì dùng try-catch trong từng controller. Một class <code>@RestControllerAdvice</code> với các method <code>@ExceptionHandler</code> ánh xạ mỗi loại exception tới đúng HTTP status và một body lỗi nhất quán, và từ Spring 6 có thể dùng định dạng <code>ProblemDetail</code> theo RFC 7807 cho body đó. Các quy tắc chính: biến các lỗi domain và lỗi validation thành các mã 4xx rõ ràng, không bao giờ gửi stack trace hay chi tiết nội bộ cho client, log toàn bộ chi tiết ở phía server kèm một correlation id, và để các lỗi thực sự ngoài dự kiến trở thành lỗi 500. Cách này giữ controller gọn gàng và các phản hồi lỗi đồng nhất trên toàn API.</p></details>
...
<details class="viet-answer"><summary>🇻🇳 Đáp án (Tiếng Việt)</summary><p><strong>AOP</strong> đưa các mối quan tâm dùng chung như transaction, security, logging và caching vào các aspect riêng biệt, nhờ đó chúng không bị lặp lại trên nhiều method. Các thuật ngữ chính là join point, pointcut, advice và aspect. Spring hiện thực AOP bằng proxy: JDK dynamic proxy cho interface và CGLIB subclass trong các trường hợp còn lại, bọc advice quanh bean. Vì mô hình proxy này, nó chỉ ảnh hưởng tới các lời gọi đi <em>qua</em> proxy, nên một method gọi method khác trên <code>this</code> sẽ không được advise, và nó chỉ hoạt động với các bean do Spring quản lý. Cần đến AspectJ weaving đầy đủ để advise các private method hoặc các object thường.</p></details>
...
<details class="viet-answer"><summary>🇻🇳 Đáp án (Tiếng Việt)</summary><p>Auto-configuration thiết lập các bean mặc định hợp lý dựa trên những gì có trên classpath, nhưng luôn cho phép developer ghi đè chúng. Bên trong, <code>@EnableAutoConfiguration</code> nạp một danh sách các config class từ file <code>AutoConfiguration.imports</code>, và mỗi class được bảo vệ bởi các kiểm tra <code>@Conditional</code> như <code>@ConditionalOnClass</code>, <code>@ConditionalOnMissingBean</code> và <code>@ConditionalOnProperty</code>. Ví dụ, có H2 trên classpath sẽ thiết lập một datasource in-memory, nhưng khi bạn định nghĩa bean của riêng mình, <code>@ConditionalOnMissingBean</code> sẽ ưu tiên bean của bạn. Chạy với <code>--debug</code> để xem báo cáo về các điều kiện và biết vì sao một config được hay không được áp dụng.</p></details>
...
<details class="viet-answer"><summary>🇻🇳 Đáp án (Tiếng Việt)</summary><p>Propagation quyết định một method transactional sẽ làm gì khi được gọi trong hoặc ngoài một transaction đang tồn tại. <strong>REQUIRED</strong> (mặc định) tham gia vào transaction hiện tại hoặc tạo một cái mới. <strong>REQUIRES_NEW</strong> tạm dừng transaction bên ngoài và chạy trên transaction riêng của nó, hữu ích cho những việc như audit log phải được lưu ngay cả khi tác vụ chính thất bại. <strong>NESTED</strong> dùng một savepoint. Lưu ý rằng <code>@Transactional</code> hoạt động thông qua một proxy, nên gọi method từ bên trong cùng một class sẽ bỏ qua nó.</p></details>
<ul>
...
<details class="viet-answer"><summary>🇻🇳 Đáp án (Tiếng Việt)</summary><p><strong>Vấn đề N+1</strong> xảy ra khi nạp một danh sách N bản ghi cha theo kiểu lazy sẽ chạy thêm một truy vấn cho mỗi bản ghi cha để nạp dữ liệu liên quan, biến một truy vấn thành N+1 và làm giảm hiệu năng. Các cách khắc phục phổ biến là <code>JOIN FETCH</code> hoặc <code>@EntityGraph</code> để nạp quan hệ trong một truy vấn, hoặc batch loading với <code>@BatchSize</code> để biến N truy vấn thành vài truy vấn IN. Fetching <strong>EAGER</strong> không phải là giải pháp, vì nó nạp quan hệ ở mọi nơi và có thể gây ra các dòng trùng lặp. Mặc định tốt là LAZY cộng với việc fetch tường minh cho từng use case, hoặc một DTO projection, và giữ show-sql bật trong lúc phát triển để phát hiện sớm.</p></details>
...
<details class="viet-answer"><summary>🇻🇳 Đáp án (Tiếng Việt)</summary><p>Cả hai đều ngăn mất cập nhật (lost update) nhưng theo cách khác nhau. <strong>Optimistic locking</strong> dùng một cột <code>@Version</code> và không giữ khóa nào ở database. Khi lưu, nó kiểm tra version; nếu một transaction khác đã thay đổi trước, bạn nhận được exception và phải retry. Cách này tốt nhất khi xung đột hiếm khi xảy ra, như đa số ứng dụng web. <strong>Pessimistic locking</strong> khóa ngay dòng dữ liệu ở database bằng <code>SELECT ... FOR UPDATE</code>, nên những transaction khác phải chờ. Chỉ dùng nó cho các dòng dữ liệu bị tranh chấp cao như tồn kho hay số dư tài khoản. Quy tắc đơn giản: optimistic cho tranh chấp thấp, pessimistic cho tranh chấp cao trên các transaction ngắn.</p></details>
...
<details class="viet-answer"><summary>🇻🇳 Đáp án (Tiếng Việt)</summary><p>Bước ngoặt lớn là Spring 6 và Boot 3. Chúng yêu cầu tối thiểu <strong>Java 17</strong>, và thay đổi phá vỡ nhiều code nhất là việc chuyển tên package từ <code>javax.*</code> sang <code>jakarta.*</code>, nên tất cả import của servlet, JPA và validation đều thay đổi. Chúng cũng bổ sung observability tích hợp sẵn với Micrometer, <code>ProblemDetail</code> theo chuẩn RFC 7807, và hỗ trợ native-image thông qua Spring AOT và GraalVM. Một dự án migration thực tế nghĩa là chuyển lên Java 17, đổi tên sang jakarta, nâng cấp toàn bộ thư viện lên phiên bản tương thích Jakarta, và thay thế phần cấu hình security cũ. Về cơ bản đây là một đợt nâng cấp dependency và namespace được phối hợp đồng bộ, chứ không phải viết lại toàn bộ.</p></details>
...
<details class="viet-answer"><summary>🇻🇳 Đáp án (Tiếng Việt)</summary><p>Ba pattern này mỗi cái giải quyết một bài toán khác nhau trong hệ phân tán. <strong>Circuit Breaker</strong> (Resilience4j) ngăn lỗi lan truyền dây chuyền: sau khi gọi tới một dependency thất bại quá nhiều lần, nó mở (open) và fail ngay lập tức, rồi chuyển sang half-open để thử xem dependency đã hồi phục chưa. <strong>Saga</strong> giữ dữ liệu nhất quán giữa các service mà không cần transaction phân tán, bằng một chuỗi các local transaction kèm các hành động bù trừ (compensating) để hoàn tác, được điều phối bằng sự kiện (choreography) hoặc bằng một bộ điều phối trung tâm (orchestration). <strong>CQRS</strong> tách model ghi khỏi model đọc để mỗi bên có thể scale và được thiết kế riêng, thường đi kèm với event sourcing. Cả ba đều làm tăng đáng kể độ phức tạp vận hành, nên chỉ dùng khi bài toán cụ thể đó thực sự tồn tại.</p></details>
...
<details class="viet-answer"><summary>🇻🇳 Đáp án (Tiếng Việt)</summary><p>Code chất lượng production chủ yếu là ở chỗ rõ ràng, an toàn và dễ test. Ưu tiên đối tượng bất biến (immutable) và các class nhỏ chỉ làm một việc, lập trình hướng tới interface, và inject dependency. Hãy validate đầu vào ngay từ sớm, tránh trả về <code>null</code> (dùng <code>Optional</code> hoặc collection rỗng), và đừng bao giờ che giấu exception. Luôn dùng try-with-resources cho các resource, và thêm logging cùng metrics để bạn thấy được code đang làm gì. Hãy dùng thư viện chuẩn và các thư viện đáng tin cậy thay vì tự viết, và bao phủ code bằng test, phân tích tĩnh (static analysis) và code review.</p></details>
