...
<details class="viet-answer"><summary>🇻🇳 Đáp án (Tiếng Việt)</summary><p>REST là một phong cách kiến trúc được định nghĩa bằng các ràng buộc, chứ không phải cách gọi khác của "JSON qua HTTP": giao diện đồng nhất, không lưu trạng thái, tách client–server, có thể cache, hệ thống phân lớp, và tuỳ chọn thêm code-on-demand. Cái mà hầu hết đội ngũ thực sự làm ra là "HTTP API" — URL có dạng tài nguyên và payload JSON — trong khi lặng lẽ vi phạm hai ràng buộc. Thứ nhất là giao diện đồng nhất, vì gần như không ai làm hypermedia, nên client cứ hard-code sẵn mẫu URL thay vì đi theo link trả về. Thứ hai, và tai hại hơn nhiều, là tính không lưu trạng thái: chỉ cần ai đó nhét session hay trạng thái của một luồng nhiều bước vào bộ nhớ server là API lập tức mất khả năng scale ngang mà không ai hay. Quan điểm thật của tôi khi đi phỏng vấn là: làm REST đầy đủ kể cả HATEOAS thường không đáng công với một API nội bộ, nhưng tính không lưu trạng thái và việc dùng đúng ngữ nghĩa HTTP thì không thể thoả hiệp — chính hai thứ đó cho bạn cân bằng tải, cache, retry và proxy gần như miễn phí.</p></details>
<table>
...
<details class="viet-answer"><summary>🇻🇳 Đáp án (Tiếng Việt)</summary><p><strong>Safe</strong> nghĩa là request không làm đổi trạng thái phía server, nên trình thu thập dữ liệu hay trình duyệt tải trước có thể gọi thoải mái. <strong>Idempotent</strong> nghĩa là gọi N lần thì trạng thái server cuối cùng vẫn giống hệt gọi một lần — chú ý rằng nó chỉ nói về trạng thái, không hề hứa hẹn gì về nội dung response. <strong>Cacheable</strong> nghĩa là response được phép lưu lại và dùng lại. GET và HEAD vừa safe, vừa idempotent, vừa cacheable; PUT và DELETE thì idempotent nhưng không safe; POST mặc định không thuộc nhóm nào; còn PATCH chỉ idempotent nếu bạn cố ý thiết kế cho nó như vậy. Chuyện này cực kỳ quan trọng khi chạy thật, bởi mọi tầng nằm giữa client và server của bạn — trình duyệt, CDN, load balancer, service mesh, thư viện HTTP client — đều sẽ tự động gửi lại các request idempotent khi gặp timeout mà chẳng hỏi ý bạn. Nên nếu bạn giấu một thao tác thay đổi dữ liệu sau GET, hay viết một PUT không idempotent, thì bạn không chỉ phá vỡ quy ước: bạn đã tạo ra những lần trừ tiền trùng và những bản ghi ma cực kỳ khó tái hiện.</p></details>
<table>
...
<details class="viet-answer"><summary>🇻🇳 Đáp án (Tiếng Việt)</summary><p>Vấn đề gốc là client không bao giờ phân biệt được giữa "request của tôi chưa hề tới nơi" và "nó đã tới, đã chạy xong, chỉ có response bị mất trên đường về"; nên khi gặp timeout, nó chỉ còn hai lựa chọn đều dở: gửi lại và chấp nhận nguy cơ trùng, hoặc bỏ cuộc và chấp nhận nguy cơ mất đơn hàng. Lời giải mà cả ngành đang dùng — Stripe và hầu hết các API thanh toán đều vậy — là idempotency key: client tự sinh một khoá duy nhất cho mỗi thao tác nghiệp vụ, gửi kèm trong header, và server cam kết rằng request đầu tiên mang khoá đó sẽ được thực thi, còn mọi request sau đều nhận lại đúng kết quả cũ chứ không chạy lại. Chi tiết cài đặt phân biệt một thiết kế chạy được với một thiết kế hỏng là: bạn phải ghi khoá đó <em>trong cùng một transaction</em> với tác động nghiệp vụ — đặt ràng buộc unique lên cột khoá là cách làm hộ bạn việc này — nếu không, hai lần retry chạy song song sẽ cùng vượt qua bước kiểm tra "tôi đã thấy khoá này chưa?" và cùng thực thi. Bạn cũng cần lưu lại response, quy định thời hạn sống của khoá, và quyết định xử lý ra sao khi cùng một khoá lại đi kèm payload khác — trường hợp này nên trả 422, vì gần như luôn là lỗi phía client.</p></details>
<pre>POST /payments
...
<details class="viet-answer"><summary>🇻🇳 Đáp án (Tiếng Việt)</summary><p>Cái bẫy của câu hỏi này nằm ở chữ "mobile": khác với client web, bạn không thể vá một phát rồi sáng mai toàn bộ người dùng đều chạy bản mới — các phiên bản app cũ sống ngoài đời hàng năm trời, và có những người dùng không bao giờ cập nhật. Vậy nên đổi kiểu dữ liệu ngay tại chỗ là phương án phải loại, dù bạn triển khai cẩn thận tới đâu. Cách làm đúng là "mở rộng rồi thu hẹp": thêm một trường mới với kiểu mới đặt cạnh trường cũ, trong giai đoạn chuyển tiếp thì ghi dữ liệu cho cả hai, hướng client mới sang trường mới, đo xem còn ai đang đọc trường cũ, và chỉ gỡ bỏ khi con số đó về 0 hoặc phiên bản app đó đã hết hỗ trợ. Nếu hai trường không thể cùng tồn tại một cách mạch lạc — chẳng hạn ý nghĩa đã đổi chứ không chỉ đổi kiểu — thì hãy đánh version cho cả phần biểu diễn tài nguyên thay vì cho từng trường. Cũng nên nói thẳng rằng tình huống này thường là triệu chứng: nó cho thấy API đang phơi thẳng mô hình nội bộ ra ngoài, nên cách chữa về lâu dài là dựng một lớp DTO cho response để tách định dạng truyền đi khỏi cấu trúc bảng trong cơ sở dữ liệu.</p></details>
...
<details class="viet-answer"><summary>🇻🇳 Đáp án (Tiếng Việt)</summary><p>Giới hạn tần suất tồn tại để bảo vệ dịch vụ trước bất kỳ bên gọi đơn lẻ nào, nên quyết định đầu tiên là lấy gì làm khoá đếm — và câu trả lời gần như luôn là danh tính đã xác thực: API key, client ID hay user ID, chứ không phải địa chỉ IP; bởi NAT của doanh nghiệp và nhà mạng di động gom hàng nghìn người dùng hợp lệ vào chung một IP, trong khi kẻ tấn công lại đổi IP thoải mái trong dải địa chỉ của một nhà cung cấp đám mây. Chọn thuật toán là một bài đánh đổi: cửa sổ cố định thì cực đơn giản nhưng cho phép một cú dồn gấp đôi hạn mức ngay chỗ giáp ranh hai cửa sổ; cửa sổ trượt chữa được điều đó nhưng tốn kém hơn; còn token bucket là thứ tôi thường chọn, vì nó cho phép dồn một chút có kiểm soát trong khi vẫn ép được mức trung bình dài hạn — đúng với cách các client thật vận hành. Hình hài của response cũng quan trọng ngang với việc chặn: hãy trả 429 kèm header <code>Retry-After</code> và bộ header <code>RateLimit</code>, để một client cư xử tử tế có thể tự giảm nhịp thay vì cứ dội vào bạn; và hãy đảm bảo việc từ chối là rẻ — chặn ngay ở gateway hoặc ở biên, đừng chặn sau khi đã trót truy vấn cơ sở dữ liệu xong xuôi.</p></details>
<table>
...
<details class="viet-answer"><summary>🇻🇳 Đáp án (Tiếng Việt)</summary><p>Bạn phải chấp nhận rằng không thể có transaction phân tán, và thay vào đó thiết kế cho nhất quán cuối cùng. Two-phase commit xuyên qua các service HTTP thì về lý thuyết là làm được nhưng trên thực tế là một sai lầm — nó ràng buộc tính sẵn sàng của các bên vào nhau, giữ khoá xuyên suốt các lệnh gọi mạng, và chẳng nền tảng phổ biến nào hỗ trợ nó cho ra hồn. Hai mẫu thiết kế dùng được là saga và outbox. <strong>Saga</strong> chia thao tác thành nhiều transaction cục bộ, mỗi bước đi kèm một hành động bù trừ; nếu bước thứ ba hỏng thì chạy phần bù cho bước hai và bước một — lưu ý rằng bù trừ không phải là rollback, vì trạng thái trung gian đã từng hiển thị ra ngoài, nên "huỷ đơn hàng" là một sự kiện nghiệp vụ mới chứ không phải một thao tác xoá dấu vết. <strong>Transactional outbox</strong> giải quyết vấn đề tinh vi hơn nằm bên dưới: bạn không thể vừa ghi vào cơ sở dữ liệu vừa phát sự kiện một cách nguyên tử; nên thay vào đó bạn ghi sự kiện vào một bảng outbox trong cùng transaction, rồi để một tiến trình riêng đọc bảng đó mà phát đi — nhờ vậy bài toán nguyên tử phân tán biến thành một bài toán cục bộ cộng thêm cơ chế giao ít nhất một lần. Từ đó, mọi bên tiêu thụ đều bắt buộc phải idempotent; và phần thành thật nhất của câu trả lời này là thừa nhận rằng phía nghiệp vụ mới là bên phải quyết định điều gì xảy ra trong khoảng thời gian hai service còn đang lệch nhau.</p></details>
...
<details class="viet-answer"><summary>🇻🇳 Đáp án (Tiếng Việt)</summary><p>Câu hỏi thật sự đáng bàn không phải là bạn sinh ra thứ nào trước, mà là có thứ gì đổ vỡ hay không khi hai bên lệch nhau — một bản đặc tả mà không có gì kiểm chứng thì chỉ là tài liệu, và tài liệu thì sẽ mục. <strong>Contract-first</strong> nghĩa là tài liệu OpenAPI là nguồn chân lý, được duyệt trước khi viết code, rồi sinh ra khung server và SDK cho client từ đó; cách này rõ ràng là đúng khi nhiều đội hoặc bên tiêu thụ bên ngoài cần làm song song, vì bản hợp đồng trở thành điểm phối hợp chung. <strong>Code-first</strong> — sinh đặc tả từ các annotation trong code — ít ma sát hơn hẳn trong công việc hằng ngày và giữ cho đặc tả tự động khớp với code, nên phần lớn service nội bộ do một đội làm đều chọn cách này. Chọn hướng nào cũng được, nhưng cách chống lệch thì giống nhau và bắt buộc phải nằm trong CI: kiểm tra rằng bản đặc tả sinh ra hay được commit khớp với phần cài đặt thật, chạy contract test hướng bên tiêu thụ để một thay đổi phía nhà cung cấp làm hỏng một client thật sẽ đánh trượt luôn bản build của nhà cung cấp, và so sánh khác biệt của đặc tả ở mỗi pull request để một thay đổi phá vỡ tương thích hiện ra ngay lúc review chứ không phải lúc đã lên môi trường thật.</p></details>
<table>
...
<details class="viet-answer"><summary>🇻🇳 Đáp án (Tiếng Việt)</summary><p>HATEOAS nghĩa là response mang theo các đường dẫn mô tả những việc bạn có thể làm tiếp theo, để client khám phá các bước chuyển hợp lệ ngay lúc chạy thay vì gắn cứng URL và luật nghiệp vụ vào code. Lợi ích thật của nó là giảm ràng buộc: server có thể dời một URI hay thay đổi những hành động được phép ở một trạng thái nào đó, mà client biết đi theo link thì vẫn chạy bình thường — một đơn hàng đã giao thì đơn giản là không kèm link huỷ nữa, nhờ vậy máy trạng thái chỉ nằm ở một chỗ thay vì bị chép lại trong từng client. Quan điểm thật lòng của tôi: tôi sẽ không làm HATEOAS đầy đủ cho một API nội bộ hay API dùng trong nội bộ tổ chức, bởi gần như không client nào được viết để đi theo link cả — lập trình viên đọc tài liệu, gõ cứng đường dẫn vào code, và phần hypermedia trở thành dữ liệu thừa chẳng ai dùng trong khi đám test của bạn vẫn phải nuôi nó. Chỗ nó thật sự đáng đồng tiền là những API công khai sống lâu với nhiều bên tiêu thụ độc lập mà bạn không thể điều phối, và những miền nghiệp vụ nặng về quy trình, nơi câu hỏi "ngay lúc này tôi làm được gì với tài nguyên này" thật sự thay đổi liên tục. Ngoài ra còn một lối đi giữa rẻ tiền mà tôi có dùng: trả kèm danh sách hành động hay bước chuyển khả dụng ở trạng thái hiện tại, mà không cần cam kết làm hypermedia đầy đủ — cách này gom được phần lớn giá trị (giao diện thôi phải tự cài lại máy trạng thái) với chi phí chỉ bằng một phần nhỏ.</p></details>
