...
<details class="viet-answer"><summary>🇻🇳 Đáp án (Tiếng Việt)</summary><p><code>git rebase -i</code> cho phép bạn viết lại các commit chưa push của chính mình thành một mạch truyện mà người review đọc được: <strong>reword</strong> để sửa message, <strong>squash</strong>/<strong>fixup</strong> để gộp sáu commit "wip" và "fix typo" vào đúng commit của nó, <strong>edit</strong> để tách hoặc sửa một commit, <strong>drop</strong> để bỏ một commit sai, và sắp lại thứ tự sao cho phần refactor đứng trước phần đổi hành vi. Mẹo làm việc chuyên nghiệp là tạo sẵn các commit fixup có chỉ đích ngay khi làm: commit với <code>--fixup=&lt;sha&gt;</code> rồi chạy <code>git rebase -i --autosquash</code>, Git sẽ tự xếp đúng chỗ. Mục đích không phải để cho đẹp: mỗi commit nên build được và pass test một cách độc lập, vì đó mới là điều làm cho <code>git bisect</code>, <code>git revert</code> và việc review code thật sự hoạt động.</p></details>
...
<details class="viet-answer"><summary>🇻🇳 Đáp án (Tiếng Việt)</summary><p><code>git fetch</code> tải các object mới về và cập nhật các nhánh <em>remote-tracking</em> (<code>origin/main</code>) mà không chạm vào working tree — lệnh này luôn an toàn. <code>git pull</code> là <code>fetch</code> cộng thêm một bước tích hợp, mà mặc định bước đó là <em>merge</em> — đây chính là nguồn gốc của những commit "Merge branch 'main' of…" ngoài ý muốn; đặt <code>pull.rebase=true</code> hoặc <code>pull.ff=only</code> sẽ khiến nó dễ đoán hơn. <code>git push</code> đẩy commit của bạn lên và yêu cầu remote dịch nhánh của nó, và remote sẽ từ chối nếu việc đó không phải fast-forward. Mô hình cần nhớ: <code>origin/main</code> chỉ là bản cache ở máy bạn, ghi lại remote trông như thế nào tại lần fetch gần nhất — nên câu "nhánh của bạn đang chậm 3 commit" là thông tin từ ảnh chụp đó, không phải sự thật tức thời.</p></details>
...
<details class="viet-answer"><summary>🇻🇳 Đáp án (Tiếng Việt)</summary><p>Tag là một cái tên cố định gắn cho một commit, và chính vì vậy nó là cách đúng để đánh dấu bản phát hành. Tag <strong>lightweight</strong> chỉ là một con trỏ; tag <strong>annotated</strong> (<code>-a</code>) là một object thật, có người tạo, ngày tạo, message và có thể ký GPG/SSH — với release thì luôn dùng annotated, vì nó audit được và cũng là thứ mà <code>git describe</code> dùng. Tag không được push theo mặc định (phải <code>git push --follow-tags</code>), và theo quy ước thì tag là bất biến: đừng bao giờ dịch một tag đã công bố, vì bên tiêu thụ và hệ thống build đã cache nó. Trong thực tế thì tag là thứ khởi động quy trình phát hành: CI kích hoạt khi thấy tag <code>v*</code>, nhúng version vào artifact, sinh changelog từ Conventional Commits, rồi publish — với các công cụ như semantic-release hay Changesets tự quyết định mức tăng semver dựa trên loại commit.</p></details>
