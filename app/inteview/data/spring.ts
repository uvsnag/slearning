...
<details class="viet-answer"><summary>🇻🇳 Đáp án (Tiếng Việt)</summary><p>AOP tách các cross-cutting concern như logging, security, transaction, caching và retry ra khỏi code nghiệp vụ và đưa vào các aspect bao quanh lời gọi method bằng các advice như <code>@Around</code>. Nhiều annotation quen thuộc, bao gồm <code>@Transactional</code>, <code>@Cacheable</code>, <code>@Async</code> và <code>@PreAuthorize</code>, đều được xây dựng dựa trên AOP. Spring AOP dùng proxy, JDK dynamic proxy cho interface và CGLIB cho class. Chính vì vậy, việc gọi một method đã được advise từ bên trong cùng một object sẽ đi vòng qua proxy và bỏ qua phần advice.</p></details>
