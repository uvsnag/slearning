...
<details class="viet-answer"><summary>🇻🇳 Đáp án (Tiếng Việt)</summary><p>Next.js là một framework React full-stack, trong khi Create React App chỉ là một công cụ bundle phía client và hiện đã bị deprecated. Next.js bổ sung server rendering, routing dựa trên file, API route handler, cùng khả năng tối ưu image và bundle sẵn có. Điều này giúp SEO tốt hơn và tải lần đầu nhanh hơn, vì CRA gửi về một trang HTML rỗng. Đánh đổi là bạn phải học nhiều khái niệm hơn và cần một server để chạy, nên với một ứng dụng nội bộ nhỏ thì một bản build thuần client vẫn có thể đơn giản hơn.</p></details>
<ul>
...
<details class="viet-answer"><summary>🇻🇳 Đáp án (Tiếng Việt)</summary><p>App Router là cách tiếp cận mới hơn, được xây dựng quanh React Server Components, nested layout không bị re-render khi điều hướng, cùng các file <code>loading.tsx</code> và <code>error.tsx</code> có sẵn. Pages Router vẫn hoạt động nhưng đã bị đóng băng về tính năng mới và dùng <code>getServerSideProps</code> cùng <code>getStaticProps</code>. Thay đổi chính là trong App Router, chiến lược render đến từ cách bạn fetch dữ liệu, chứ không phải từ một hàm export đặc biệt. Dự án mới nên dùng App Router; còn các app Pages hiện có thì không cần migrate nếu không có lý do rõ ràng.</p></details>
...
<details class="viet-answer"><summary>🇻🇳 Đáp án (Tiếng Việt)</summary><p>Routing dựa trên cấu trúc file: một thư mục có <code>page.tsx</code> sẽ trở thành một route, <code>[id]</code> là một dynamic segment, <code>[...slug]</code> là catch-all, còn <code>[[...slug]]</code> là optional catch-all. Route group đặt trong dấu ngoặc đơn giúp tổ chức file mà không thay đổi URL, còn parallel route với slot ký hiệu at-sign cho phép render nhiều phần trong cùng một layout, chẳng hạn như modal. Các trang dynamic có thể được pre-render tại thời điểm build bằng <code>generateStaticParams</code>. Intercepting route hỗ trợ mô hình mở nội dung trong một modal nhưng vẫn là một link thật sự.</p></details>
...
<details class="viet-answer"><summary>🇻🇳 Đáp án (Tiếng Việt)</summary><p>Middleware chạy tại edge trước mỗi request được khớp, nên đây là nơi phù hợp cho các logic dùng chung nhẹ nhàng như redirect xác thực, định tuyến theo locale hoặc vị trí địa lý, A/B testing và thiết lập header. Nó phải giữ nhẹ vì edge runtime có API hạn chế, nên cần tránh các tác vụ nặng như gọi database. Nên dùng <code>matcher</code> để bỏ qua các file tĩnh và API route. Middleware là một cổng chặn nhanh chứ không phải bước kiểm tra bảo mật thực sự, nên việc phân quyền thật sự vẫn phải diễn ra bên trong handler.</p></details>
...
<details class="viet-answer"><summary>🇻🇳 Đáp án (Tiếng Việt)</summary><p>Một hydration mismatch nghĩa là HTML được render trên server không khớp với lần render đầu tiên trong trình duyệt, khiến React bỏ đi phần markup từ server và render lại, việc này chậm và có thể gây nháy nội dung sai. Các nguyên nhân thường gặp là những giá trị không xác định như <code>Date.now()</code> hay <code>Math.random()</code>, việc định dạng theo locale và múi giờ, cùng việc đọc các API chỉ có ở trình duyệt như <code>localStorage</code> trong lúc render. Cách khắc phục là render những giá trị đó sau khi mount bên trong <code>useEffect</code> để lần render đầu tiên khớp nhau ở cả hai phía. Chỉ dùng <code>suppressHydrationWarning</code> cho một node đơn lẻ không thể tránh khỏi như một timestamp, hoặc tắt SSR cho một widget phụ thuộc hoàn toàn vào trình duyệt.</p></details>
...
<details class="viet-answer"><summary>🇻🇳 Đáp án (Tiếng Việt)</summary><p>Dữ liệu cũ chỉ xuất hiện trên production thường là do vấn đề caching, vì <code>next dev</code> bỏ qua hầu hết các cache. App Router có bốn loại cache: request memoization trong một lần render, Data Cache dùng chung giữa các request, Full Route Cache của các route tĩnh đã render, và Router Cache phía client. Một trang chỉ chứa các fetch có thể cache sẽ được render tĩnh tại thời điểm build và giữ nguyên dữ liệu cũ cho tới khi có gì đó revalidate nó. Hãy kiểm soát bằng <code>cache: 'no-store'</code> hoặc <code>revalidate</code> trên từng fetch, cấu hình route, cùng <code>revalidateTag</code> hoặc <code>revalidatePath</code> sau khi có thay đổi, và luôn kiểm thử bằng <code>next build</code>. Lưu ý rằng Next.js 15 đã đổi fetch thành không cache theo mặc định.</p></details>
...
<details class="viet-answer"><summary>🇻🇳 Đáp án (Tiếng Việt)</summary><p>Server Action là các hàm <code>'use server'</code> mà client gọi thông qua một HTTP POST được tạo tự động, cho phép thực hiện mutation mà không cần tự viết API route, kèm theo progressive enhancement trong form. Mỗi action trở thành một endpoint công khai với một ID cố định, nên việc ẩn nút trên giao diện chẳng bảo vệ được gì vì ai cũng có thể tự gửi cùng một POST đó. Mỗi action nên được đối xử như một API route công khai: kiểm tra xác thực, kiểm tra phân quyền và validate đầu vào ngay bên trong thân của action. Ở phía client, <code>useFormStatus</code> và <code>useActionState</code> xử lý trạng thái pending và lỗi.</p></details>
...
<details class="viet-answer"><summary>🇻🇳 Đáp án (Tiếng Việt)</summary><p>Core Web Vitals là ba chỉ số đo trải nghiệm người dùng thực của Google: LCP cho tốc độ tải (mục tiêu dưới 2.5s), INP (đã thay thế FID) cho khả năng phản hồi (mục tiêu dưới 200ms), và CLS cho độ ổn định thị giác (mục tiêu dưới 0.1). Chúng quan trọng vì ảnh hưởng đến thứ hạng tìm kiếm và được đo trên người dùng thực. Các cách khắc phục phổ biến là preload image LCP, luôn đặt kích thước image hoặc aspect-ratio để tránh layout shift, và chia các tác vụ JavaScript dài thành các phần nhỏ hơn. Đây là các con số field ở p75 từ người dùng thực, nên chỉ có điểm Lighthouse tốt thôi thì chưa đảm bảo là đạt.</p></details>
...
<details class="viet-answer"><summary>🇻🇳 Đáp án (Tiếng Việt)</summary><p>INP đo độ trễ của các tương tác trong suốt vòng đời của trang và báo cáo khoảng tương tác tệ nhất, được chia thành input delay, thời gian xử lý và presentation delay, với mục tiêu dưới 200ms. Nó thay thế FID vì FID chỉ đo độ trễ của tương tác đầu tiên và bỏ qua chi phí của handler cùng phần vẽ lại, nên các trang có thể đạt FID mà vẫn cảm thấy chậm ở mỗi lần click. Cách khắc phục là hiển thị phản hồi ngay lập tức, rồi nhường main thread và chia công việc nặng thành từng đoạn nhỏ; trong React, hãy giữ cho bản cập nhật cấp bách chạy nhanh và bọc bản cập nhật tốn kém trong <code>startTransition</code>. Vì INP là một chỉ số field, nó nên được debug từ dữ liệu người dùng thực ở p75 theo từng nhóm thiết bị, chứ không phải trên một máy nhanh.</p></details>
...
<details class="viet-answer"><summary>🇻🇳 Đáp án (Tiếng Việt)</summary><p>Thay vì đoán mò, hãy chia LCP thành bốn phần của nó: TTFB, độ trễ trước khi resource bắt đầu tải, thời gian tải resource, và render delay, những thứ mà tab Performance hoặc web-vitals attribution có thể cho thấy. Hãy khắc phục theo thứ tự mức độ tác động: cải thiện TTFB trước, làm cho image LCP dễ được tìm thấy sớm bằng một preload cùng <code>fetchpriority='high'</code>, thu nhỏ nó bằng AVIF hoặc WebP và một srcset có kích thước phù hợp, rồi mới loại bỏ những thứ chặn render. Các vấn đề tự gây ra thường gặp là lazy-load image hero, ẩn nó dưới dạng CSS background nên bị tìm thấy muộn, hoặc render nó bằng JavaScript phía client. Nếu một preload không tạo ra khác biệt nào, hãy kiểm tra network waterfall thực tế thay vì tin vào một checklist.</p></details>
...
<details class="viet-answer"><summary>🇻🇳 Đáp án (Tiếng Việt)</summary><p>JavaScript là loại byte đắt đỏ nhất vì nó được parse, biên dịch và chạy trên main thread, nên có ba cách để giảm kích thước bundle. Chia code theo route và tính năng để người dùng chỉ tải về những gì họ dùng, giữ các dependency ở dạng ES module với <code>sideEffects</code> được khai báo để tree shaking có thể loại bỏ code chết, và tránh cái bẫy barrel-file khi import từ một <code>index.ts</code> lớn kéo theo cả đồ thị module. Một ví dụ rõ ràng là import toàn bộ <code>lodash</code> so với import từ <code>lodash-es</code>. Hãy luôn xác nhận kết quả bằng một bundle analyzer để bắt được các dependency bị trùng lặp hoặc một SDK bị nhét vào nguyên cả gói do sơ suất.</p></details>
...
<details class="viet-answer"><summary>🇻🇳 Đáp án (Tiếng Việt)</summary><p>Web font gây hại cho hai chỉ số cùng lúc: chúng có thể gây chữ vô hình (FOIT) trong tối đa ba giây, và có thể gây layout shift (CLS) khi việc chuyển sang font thật làm bố cục trang bị dồn lại. Một thiết lập đúng dùng WOFF2 tự host với <code>font-display: swap</code> để chữ luôn hiển thị, một preload kèm <code>crossorigin</code> cho đúng một file font quan trọng, và một fallback khớp về metric bằng <code>size-adjust</code> cùng <code>ascent-override</code> để việc swap không làm dịch chuyển gì cả. Đây chính xác là những gì <code>next/font</code> tự động hóa, bao gồm cả việc tự host font Google. Với giao diện ứng dụng, cũng đáng để tự hỏi liệu có thật sự cần một font thương hiệu hay không so với một stack font hệ thống không tốn chi phí nào.</p></details>
...
<details class="viet-answer"><summary>🇻🇳 Đáp án (Tiếng Việt)</summary><p>Quy tắc cốt lõi là các asset đã gắn hash nội dung sẽ không bao giờ thay đổi, nên có thể cache chúng trong một năm với <code>immutable</code>, còn điểm vào HTML vốn đặt tên cho chúng thì không bao giờ được tin là còn mới và dùng <code>no-cache</code> kèm một ETag để nó revalidate với chi phí thấp. Một bản deploy hỏng thường gặp xảy ra khi đặt <code>max-age</code> dài trên <code>index.html</code>: các file hash mới được ship, các file cũ bị xóa, và người dùng đang dùng HTML đã cache sẽ gặp lỗi 404 với một script bị thiếu cùng một màn hình trắng. Một tab cũ còn mở đang tải một chunk đã bị xóa cũng gặp đúng vấn đề đó. Hãy giảm thiểu bằng cách giữ lại các asset của bản deploy trước trong một khoảng thời gian ân hạn và khôi phục từ một <code>ChunkLoadError</code> bằng cách reload.</p></details>
