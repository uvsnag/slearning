...
<details class="viet-answer"><summary>🇻🇳 Đáp án (Tiếng Việt)</summary><p>Các loại JOIN trong SQL được phân nhóm dựa trên việc chúng giữ lại những dòng nào. <code>INNER</code> chỉ giữ những dòng khớp ở cả hai bảng, còn <code>LEFT</code> giữ mọi dòng của bảng bên trái và điền <code>NULL</code> cho phía bên phải. <code>RIGHT</code> là bản đối xứng của <code>LEFT</code>, còn <code>FULL OUTER</code> giữ dòng từ cả hai phía, tuy nhiên MySQL không hỗ trợ trực tiếp. <code>CROSS</code> tạo ra mọi cặp có thể có, và <code>SELF JOIN</code> nối một bảng với chính nó, ví dụ như nối một nhân viên với người quản lý của họ.</p></details>
<ul>
...
<details class="viet-answer"><summary>🇻🇳 Đáp án (Tiếng Việt)</summary><p><code>WHERE</code> lọc các dòng trước khi gom nhóm, còn <code>HAVING</code> lọc các nhóm sau khi đã tính tổng hợp. Vì vậy, các hàm tổng hợp như <code>COUNT</code> và <code>SUM</code> chỉ có thể dùng trong <code>HAVING</code>. Để chạy nhanh hơn, nên đưa càng nhiều điều kiện càng tốt vào <code>WHERE</code> để giảm số dòng cần gom nhóm.</p></details>
<ul>
...
<details class="viet-answer"><summary>🇻🇳 Đáp án (Tiếng Việt)</summary><p>SQL thực thi theo một thứ tự logic khác với thứ tự khi viết câu lệnh. Các bước lần lượt là <code>FROM</code> và các phép join, rồi tới <code>WHERE</code>, <code>GROUP BY</code>, <code>HAVING</code>, <code>SELECT</code>, <code>DISTINCT</code>, <code>ORDER BY</code>, và cuối cùng là <code>LIMIT</code>. Đây chính là lý do vì sao một alias cột được tạo trong <code>SELECT</code> không thể dùng trong <code>WHERE</code> nhưng lại có thể dùng trong <code>ORDER BY</code>.</p></details>
...
<details class="viet-answer"><summary>🇻🇳 Đáp án (Tiếng Việt)</summary><p>Window function thực hiện tính toán trên một tập các dòng mà không gộp chúng lại thành một, đây chính là điểm khác biệt so với <code>GROUP BY</code>. Ba hàm xếp hạng chỉ khác nhau ở cách xử lý các giá trị bằng nhau. <code>ROW_NUMBER</code> luôn cho số thứ tự duy nhất, <code>RANK</code> để lại khoảng trống sau khi có giá trị trùng, còn <code>DENSE_RANK</code> thì không để lại khoảng trống. Một ứng dụng phổ biến là tìm top N dòng trong mỗi nhóm bằng <code>ROW_NUMBER</code> kết hợp <code>PARTITION BY</code>.</p></details>
...
<details class="viet-answer"><summary>🇻🇳 Đáp án (Tiếng Việt)</summary><p>Một CTE đặt tên cho một kết quả tạm thời và được dùng trong phạm vi một câu lệnh duy nhất, chủ yếu để giúp câu truy vấn dễ đọc hơn và cho phép đệ quy. Subquery thì viết inline và khó tái sử dụng hơn, còn temp table được lưu trữ vật lý, có thể đánh index, và tồn tại qua nhiều câu lệnh cho các công việc nhiều bước. Một điều cần lưu ý là CTE có thể không được materialize, vì một số database sẽ inline nó, nên đừng dựa vào nó như một ranh giới về hiệu năng.</p></details>
<pre>-- CTE
...
<details class="viet-answer"><summary>🇻🇳 Đáp án (Tiếng Việt)</summary><p>Các mức isolation được định nghĩa dựa trên những vấn đề mà chúng cho phép xảy ra: dirty read, non-repeatable read, và phantom read. Read Uncommitted cho phép cả ba, Read Committed ngăn dirty read, Repeatable Read ngăn thêm cả non-repeatable read, còn Serializable ngăn tất cả. Giá trị mặc định rất quan trọng, vì PostgreSQL dùng Read Committed còn MySQL InnoDB dùng Repeatable Read. Mức isolation càng cao thì càng đúng đắn nhưng lại gây ra nhiều khóa hơn và nhiều lần retry hơn.</p></details>
...
<details class="viet-answer"><summary>🇻🇳 Đáp án (Tiếng Việt)</summary><p>Deadlock xảy ra khi hai transaction mỗi bên đang giữ một khóa mà bên kia cần, tạo thành một vòng lặp mà không bên nào có thể tiếp tục. Database tự động phát hiện điều này và hủy một transaction, nên ứng dụng sẽ nhận được lỗi và phải thực thi lại toàn bộ transaction. Cách chính để phòng tránh là luôn lấy khóa theo cùng một thứ tự, ví dụ theo <code>id</code> tăng dần. Giữ cho transaction ngắn gọn cũng có ích.</p></details>
...
<details class="viet-answer"><summary>🇻🇳 Đáp án (Tiếng Việt)</summary><p>Một function trả về một giá trị, lý tưởng là không có side effect, và có thể dùng bên trong <code>SELECT</code> hoặc <code>WHERE</code>. Một procedure dùng cho logic nhiều bước: nó có thể thay đổi dữ liệu, điều khiển transaction, và trả về nhiều tập kết quả, được gọi bằng <code>CALL</code>. Những phép tính cần nằm bên trong truy vấn thì nên là function, còn những luồng nghiệp vụ có transaction như chuyển tiền thì nên là procedure.</p></details>
<ul>
...
<details class="viet-answer"><summary>🇻🇳 Đáp án (Tiếng Việt)</summary><p><code>EXPLAIN</code> cho thấy plan mà bộ lập kế hoạch chọn cùng với các ước lượng, còn <code>EXPLAIN ANALYZE</code> thực sự chạy truy vấn và bổ sung thời gian cùng số dòng thực tế. Việc kiểm tra quan trọng nhất là xem số dòng ước lượng và thực tế có chênh lệch lớn hay không, vì điều đó thường có nghĩa là thống kê đã cũ và cần được làm mới bằng <code>ANALYZE</code>. Hãy đọc plan từ nút thụt vào sâu nhất ra ngoài, và để ý một <code>Seq Scan</code> trên bảng lớn. Dùng <code>BUFFERS</code> giúp thấy được bao nhiêu dữ liệu đến từ cache so với từ đĩa.</p></details>
...
<details class="viet-answer"><summary>🇻🇳 Đáp án (Tiếng Việt)</summary><p>Một index đáng để tạo khi một cột thường xuyên được dùng trong <code>WHERE</code>, <code>JOIN</code>, hoặc <code>ORDER BY</code>, có nhiều giá trị khác nhau, và bảng lớn nhưng chỉ trả về một phần nhỏ số dòng. Tốt hơn là nên tránh đánh index cho các bảng nhỏ, các cột có ít giá trị khác nhau như boolean, các cột hiếm khi được truy vấn, và các bảng có nhiều thao tác ghi. Mỗi index tốn dung lượng đĩa và làm chậm việc ghi, nên đó là một sự đánh đổi chứ không hề miễn phí. Hãy đo mức sử dụng trước và sau, và loại bỏ những index không bao giờ được dùng.</p></details>
...
<details class="viet-answer"><summary>🇻🇳 Đáp án (Tiếng Việt)</summary><p>Một composite index bao phủ nhiều cột và tuân theo quy tắc tiền tố trái nhất (leftmost prefix). Một index trên <code>(a, b, c)</code> hỗ trợ việc lọc trên <code>a</code>, trên <code>a</code> và <code>b</code>, hoặc trên cả ba, nhưng không hỗ trợ lọc chỉ trên <code>b</code>. Quy tắc thiết kế là đặt các cột dùng phép so sánh bằng lên trước và điều kiện range xuống cuối, vì một điều kiện range sẽ chặn việc dùng các cột phía sau. Thứ tự cột quyết định index hữu ích tới mức nào.</p></details>
...
<details class="viet-answer"><summary>🇻🇳 Đáp án (Tiếng Việt)</summary><p>Một covering index chứa đủ mọi cột mà truy vấn cần, nên database có thể trả lời truy vấn chỉ từ index mà không cần đọc bảng, điều này xuất hiện dưới dạng <code>Index Only Scan</code>. Các cột dùng để lọc và sắp xếp được đưa vào làm cột khóa, còn các cột chỉ để trả về có thể đưa vào <code>INCLUDE</code> để giữ index nhỏ gọn hơn. Cái giá phải trả là một index rộng hơn và ghi chậm hơn, nên tốt nhất chỉ nên bao phủ vài truy vấn đọc nóng. Trong PostgreSQL, một index-only scan vẫn phải kiểm tra visibility map, nên việc cập nhật nhiều gần đây có thể buộc phải đọc bảng cho đến khi <code>VACUUM</code> chạy.</p></details>
...
<details class="viet-answer"><summary>🇻🇳 Đáp án (Tiếng Việt)</summary><p>Ngoài B-tree thông thường còn có một vài loại index đặc biệt. Một partial index chỉ bao phủ những dòng thỏa một điều kiện, cho ra một index nhỏ khi các truy vấn luôn nhắm vào một tập con, và một partial unique index có thể ép các quy tắc như mỗi người dùng chỉ có một phiên đang hoạt động. Một functional index đánh index cho một biểu thức như <code>LOWER(email)</code> để tra cứu không phân biệt hoa thường, còn <code>INCLUDE</code> thêm các cột chỉ để trả về nhằm phục vụ quét covering. Các index GIN và trigram xử lý tìm kiếm kiểu chứa (contains) và full-text, còn hash index thì chỉ dùng cho phép so sánh bằng. Partial index thường bị dùng chưa đủ vì một index nhỏ hơn vừa với bộ nhớ và giữ chi phí ghi thấp.</p></details>
<ul>
...
<details class="viet-answer"><summary>🇻🇳 Đáp án (Tiếng Việt)</summary><p>Mỗi secondary index là một B-tree riêng mà database phải giữ đồng bộ ở mỗi lần ghi, nên một lần insert vào một bảng có nhiều index trở thành nhiều lần ghi, gọi là write amplification. Trong PostgreSQL, một lần update lên một cột không có index có thể là một HOT update rẻ, nhưng đánh index cho một cột nóng sẽ buộc phải bảo trì mọi index ở mỗi lần update. Các bản ghi bị xóa và cập nhật cũng để lại khoảng chết, gây ra index bloat làm chậm cả việc đọc. Trước khi thêm một index, hãy kiểm tra chắc chắn rằng nó sẽ thực sự được dùng, và loại bỏ những index không ai dùng.</p></details>
