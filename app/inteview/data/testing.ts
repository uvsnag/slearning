...
<details class="viet-answer"><summary>🇻🇳 Đáp án (Tiếng Việt)</summary><p>Một unit test tốt phải <strong>chạy nhanh, độc lập, cho kết quả nhất quán, tự kiểm chứng và được viết đúng lúc — ngay khi code vừa ra đời</strong>, đó chính là các nguyên tắc FIRST. Nó tuân theo cấu trúc <strong>Arrange-Act-Assert</strong> để người đọc nhìn một cái là thấy ngay phần chuẩn bị, một hành động duy nhất được kiểm thử và kết quả mong đợi, chứ không phải dò ngược code để đoán; và nó chỉ có <strong>một lý do duy nhất để fail</strong>: nó kiểm chứng một hành vi, chứ không phải năm hành vi. Tính chất quan trọng nhất là test phải kiểm chứng <strong>hành vi nhìn thấy được qua public API</strong> chứ không phải cách cài đặt bên trong, vì test bám vào cách cài đặt sẽ gãy mỗi lần refactor và không còn là tấm lưới an toàn nữa. Nếu một test cần database, cần mạng, cần giờ hệ thống, hay cần test khác chạy trước, thì nó không còn là unit test — và chính sự chậm chạp cùng tính chập chờn đó sẽ khiến cả bộ test sớm muộn bị bỏ xó.</p></details>
...
<details class="viet-answer"><summary>🇻🇳 Đáp án (Tiếng Việt)</summary><p>Một "unit" không nhất thiết là một class: nó là <strong>một hành vi nằm trong ranh giới mà bạn kiểm soát được</strong>, có thể là một class, cũng có thể là một nhóm nhỏ các class vốn đi liền với nhau. Từ đó sinh ra hai phong cách đều hợp lệ — test <strong>solitary</strong> mock hết mọi collaborator, và test <strong>sociable</strong> dùng collaborator thật, chỉ stub ở chỗ vượt ra khỏi tiến trình — và các dự án trưởng thành thường dùng sociable cho logic nghiệp vụ, còn solitary cho những chỗ collaborator chạy chậm hoặc cho kết quả không đoán trước được. Bên trên là integration test, chạy thật qua một ranh giới thật như database hay message broker; component test gọi vào một service qua chính API của nó và giả lập các phụ thuộc bên ngoài; contract test kiểm chứng thỏa thuận giữa bên gọi và bên cung cấp mà không cần dựng cả hai; và end-to-end test chạy xuyên toàn hệ thống. Tỷ lệ giữa các tầng là bài toán chi phí: càng lên cao thì càng chậm, càng dễ chập chờn và càng khó tìm nguyên nhân, nên hãy giữ số lượng ở tầng trên thật ít và đẩy phần chi tiết xuống tầng nhanh nhất đủ sức chứng minh nó.</p></details>
...
<details class="viet-answer"><summary>🇻🇳 Đáp án (Tiếng Việt)</summary><p>Quy trình lúc nào cũng gồm sáu bước: chọn đúng một hành vi và đặt tên test theo hành vi đó; chuẩn bị dữ liệu bằng builder thay vì một khối setup dài dòng; chỉ stub những collaborator nằm ngoài ranh giới mình kiểm soát; gọi đúng một hành động; assert vào kết quả nhìn thấy được bằng assertion nối chuỗi; rồi bổ sung các trường hợp biên dưới dạng parameterized test. Trong Java, cụ thể là JUnit 5 làm bộ khung với <code>@ExtendWith(MockitoExtension.class)</code>, Mockito để stub và AssertJ để assert — vì chuỗi <code>assertThat</code> đọc lên như một bản đặc tả và khi fail thì báo lỗi rõ hơn hẳn <code>assertEquals</code> trần. Thứ khiến mọi thứ trở nên dễ dàng là constructor injection: class cần test được tạo thẳng bằng <code>new</code> và các fake được truyền vào qua constructor, nên không cần đến Spring context và test chạy xong chỉ trong vài mili-giây. Cuối cùng, hãy thử phá code production một cách có chủ ý để chắc chắn test thật sự fail — một test không bao giờ fail được còn tệ hơn là không có test.</p></details>
...
<details class="viet-answer"><summary>🇻🇳 Đáp án (Tiếng Việt)</summary><p>JUnit 5 gồm ba module — <strong>Platform</strong> lo việc khởi chạy test, <strong>Jupiter</strong> là API mà bạn viết test lên đó, và <strong>Vintage</strong> để chạy tiếp các test JUnit 4 cũ — và phần thực sự dùng hằng ngày thì khá gọn. Một test chỉ là một method gắn <code>@Test</code> trong class không cần khai báo public; vòng đời chạy theo thứ tự <code>@BeforeAll</code>, <code>@BeforeEach</code>, test, <code>@AfterEach</code>, <code>@AfterAll</code>, và mỗi method nhận một instance mới trừ khi bạn đổi lifecycle. Ngoài ra, những thứ đáng nắm là <strong>parameterized test</strong> cho các ca kiểm thử chạy theo dữ liệu, class <strong>@Nested</strong> để nhóm test theo từng ngữ cảnh, <strong>@Tag</strong> để chọn chạy một tập con trên CI, <strong>assumption</strong> để bỏ qua test thay vì báo fail, và <strong>extension</strong> — cơ chế thay thế cho runner và rule của JUnit 4, cũng là cách các thư viện như Mockito hay Spring móc vào vòng đời test. Phần assertion có thêm các biến thể gom nhóm, kiểm tra ngoại lệ và timeout, dù phần lớn các đội vẫn chọn AssertJ cho dễ đọc.</p></details>
...
<details class="viet-answer"><summary>🇻🇳 Đáp án (Tiếng Việt)</summary><p>Test double là tên gọi chung cho bất cứ thứ gì đóng thế một collaborator thật, và năm loại khác nhau ở chỗ chúng chứa bao nhiêu hành vi và bạn assert lên cái gì. <strong>Dummy</strong> chỉ là thứ điền vào cho đủ tham số, không bao giờ được dùng đến; <strong>stub</strong> trả về sẵn những giá trị đã định để test đi được tới nhánh cần quan tâm; <strong>spy</strong> là object thật nhưng có ghi lại những gì đã bị gọi; <strong>mock</strong> được đặt trước kỳ vọng về các tương tác và sẽ báo fail nếu kỳ vọng đó không xảy ra; còn <strong>fake</strong> là một bản cài đặt rút gọn nhưng chạy thật, ví dụ repository lưu trong bộ nhớ. Khác biệt quan trọng nhất khi làm thật là <strong>kiểm chứng theo trạng thái so với kiểm chứng theo tương tác</strong>: stub và fake cho phép bạn assert lên trạng thái sau cùng nên rất dễ sống chung với refactor; còn mock assert lên chính lời gọi, và chỉ nên dùng khi bản thân lời gọi đó là yêu cầu nghiệp vụ — gửi email, publish event, trừ tiền thẻ. Lạm dụng mock sẽ đẻ ra những test chỉ chép lại code cài đặt và gãy mỗi lần code đổi.</p></details>
...
<details class="viet-answer"><summary>🇻🇳 Đáp án (Tiếng Việt)</summary><p>Mockito xoay quanh bốn thao tác: tạo double bằng <code>mock</code> hoặc <code>spy</code>, quy định hành vi bằng <code>when(...).thenReturn(...)</code>, kiểm tra tương tác bằng <code>verify</code>, và bắt lại tham số bằng <code>ArgumentCaptor</code> khi thứ quan trọng chính là giá trị đã được truyền cho collaborator. Hai quy tắc giúp tránh gần hết lỗi thường gặp: <strong>argument matcher phải dùng nhất quán</strong> — nếu một tham số đã dùng <code>any()</code> thì mọi tham số còn lại cũng phải là matcher; và <strong>với spy thì phải viết <code>doReturn(...).when(spy).method()</code></strong>, vì viết kiểu <code>when(spy.method())</code> sẽ gọi method thật trước rồi mới stub. Mockito bản mới mặc định chạy ở chế độ strict khi dùng <code>MockitoExtension</code>, nên stub khai báo mà không dùng đến sẽ làm test fail — đây là điều tốt, vì nó buộc bạn dọn sạch phần setup thừa. Method static và final vẫn mock được bằng <code>mockStatic</code> và inline mock maker, nhưng khi phải dùng đến chúng thì thường đó là dấu hiệu lẽ ra nên tiêm phụ thuộc vào thay vì đi mock.</p></details>
...
<details class="viet-answer"><summary>🇻🇳 Đáp án (Tiếng Việt)</summary><p>AssertJ thay cả mớ method assert rời rạc của JUnit bằng một điểm vào duy nhất là <code>assertThat(actual)</code>, rồi nối chuỗi các assertion phù hợp với từng kiểu dữ liệu. Điều đó quan trọng vì hai lẽ: test đọc lên như một bản đặc tả, và khi fail thì thông báo chỉ thẳng ra trường nào sai thay vì chỉ nói "expected true but was false". Với object, nó có <code>extracting</code>, <code>returns</code> và <code>usingRecursiveComparison</code> để so sánh cả cây dữ liệu mà vẫn bỏ qua được những trường sinh tự động; với collection, nó có các assertion về nội dung và thứ tự diễn đạt đúng ý bạn muốn; còn với ngoại lệ, <code>assertThatThrownBy</code> cho phép kiểm tra kiểu, thông điệp và nguyên nhân gốc chỉ trong một chuỗi. <strong>Soft assertion</strong> gom mọi lỗi trong một lần chạy để test báo hết các chỗ sai chứ không dừng ở lỗi đầu tiên, còn custom assertion giúp viết bằng đúng ngôn ngữ nghiệp vụ, kiểu <code>assertThat(order).isConfirmed()</code>. Nhưng quan trọng hơn cả thư viện là thói quen: mỗi test chỉ assert vào <strong>một kết quả</strong>, bằng assertion cụ thể nhất có thể.</p></details>
...
<details class="viet-answer"><summary>🇻🇳 Đáp án (Tiếng Việt)</summary><p>Dễ test hay không là chuyện thiết kế, không phải chuyện mẹo viết test: code khó test khi nó tự tay tạo ra phụ thuộc của mình, khi nó đọc trạng thái toàn cục, hoặc khi nó trộn lẫn việc ra quyết định với việc gây tác dụng phụ. Ba thói quen giải quyết được gần như toàn bộ chuyện đó: <strong>constructor injection</strong> để mọi collaborator đều thay thế được, <strong>tách phần logic thuần túy ra khỏi I/O</strong> để những luật quan trọng nhất có thể test bằng giá trị thuần mà không cần một test double nào, và <strong>chỉ phụ thuộc vào interface do mình tự định nghĩa</strong> ở những chỗ vượt ra khỏi tiến trình — đúng như kiến trúc ports and adapters đã chuẩn hóa. Những đầu vào không đoán trước được — thời gian, số ngẫu nhiên, id sinh mới, biến môi trường — đều phải trở thành phụ thuộc được tiêm vào, ví dụ một <code>Clock</code> hay một supplier; nếu không, test sẽ chập chờn hoặc buộc phải mock static. Một dấu hiệu rất dễ nhận ra là khối setup: nếu chuẩn bị cho một unit test mà mất hơn vài dòng thì class đó đang ôm quá nhiều việc, và cách sửa đúng là sửa thiết kế chứ không phải viết test to hơn.</p></details>
...
<details class="viet-answer"><summary>🇻🇳 Đáp án (Tiếng Việt)</summary><p>Cái gì khó test thì gần như chắc chắn đang có một phụ thuộc bị giấu đi, và cách chữa quen thuộc là lôi nó ra cho lộ diện. Thời gian trở thành một <code>Clock</code> được tiêm vào để test cố định được một mốc; số ngẫu nhiên trở thành <code>Random</code> có seed cố định hoặc một supplier được tiêm vào; id trở thành <code>Supplier&lt;UUID&gt;</code>; còn môi trường hay cấu hình trở thành tham số constructor thay vì gọi tra cứu static. Method trả về void thì kiểm thử qua tác động nhìn thấy được — trạng thái thay đổi, một lời gọi ra collaborator ở ranh giới, hoặc một event được phát ra; còn method private thì không bao giờ test trực tiếp: chúng được chạy gián tiếp qua public API, và nếu thấy điều đó bất khả thi thì chính phần logic private ấy đang đòi được tách ra thành một class riêng. Với code cũ chưa refactor an toàn được, trước hết hãy chốt hành vi hiện tại lại bằng <strong>characterization test</strong>, sau đó tạo điểm chèn (seam) để thay thế phụ thuộc, rồi mới sửa. Mock static vẫn còn đó như phương án cuối cùng, nhưng nó chậm hơn, nhạy với thứ tự chạy và chỉ che đi vấn đề thiết kế chứ không sửa được nó.</p></details>
...
<details class="viet-answer"><summary>🇻🇳 Đáp án (Tiếng Việt)</summary><p>Khi cùng một hành vi phải đúng với nhiều bộ đầu vào, việc chép method test ra thành nhiều bản là cách làm sai: <code>@ParameterizedTest</code> của JUnit 5 biến đầu vào thành dữ liệu, nhờ đó mỗi ca được báo cáo riêng với tên riêng, và khi fail thì chỉ đúng ca dữ liệu gây lỗi hiện ra. Các nguồn dữ liệu có sẵn phủ hầu hết nhu cầu — <code>@ValueSource</code> cho giá trị đơn giản, <code>@CsvSource</code> cho từng cặp đầu vào – kết quả, <code>@EnumSource</code> cho máy trạng thái, <code>@MethodSource</code> khi cần object thật và <code>@CsvFileSource</code> cho tập dữ liệu lớn — còn <code>@NullAndEmptySource</code> là cách nhanh nhất để phủ hai ca biên mà ai cũng hay quên. Đi xa hơn việc liệt kê từng ví dụ là <strong>property-based testing</strong>: các thư viện như jqwik sinh ra hàng trăm đầu vào ngẫu nhiên rồi kiểm tra một bất biến thay vì một kết quả cụ thể, và khi tìm được ca lỗi thì tự rút gọn nó về trường hợp nhỏ nhất còn tái hiện được — đây chính là cách phát hiện những điều kiện biên mà không ai nghĩ tới. Điều cần cân nhắc là parameterize cái gì: cùng hành vi mà khác dữ liệu thì nên, còn khác hành vi thì không.</p></details>
...
<details class="viet-answer"><summary>🇻🇳 Đáp án (Tiếng Việt)</summary><p>Test bất đồng bộ lúc pass lúc fail chỉ vì một lý do: nó mặc định rằng việc cần chờ đã xảy ra xong đúng vào lúc nó assert. Cách chữa là đừng bao giờ sleep và cũng đừng đoán — hoặc biến phần xử lý thành đồng bộ ngay trong test bằng cách tiêm vào một executor chạy trên cùng luồng, hoặc chờ theo điều kiện bằng thư viện như Awaitility, vốn sẽ kiểm tra lại liên tục cho đến khi assertion đúng hoặc hết thời gian chờ. Với <code>CompletableFuture</code> thì chỉ cần join kèm timeout; với <code>@Async</code> của Spring thì thay executor bằng <code>SyncTaskExecutor</code>; còn với messaging thì hãy chờ tác dụng cuối cùng nhìn thấy được, đừng chờ một lời gọi nội bộ. Còn việc kiểm thử chính tính đồng thời lại là chuyện khác: dùng <code>CountDownLatch</code> hay barrier để ép các luồng chạy song song thật, lặp lại kịch bản thật nhiều lần, rồi assert vào một bất biến chẳng hạn giá trị bộ đếm cuối cùng — luôn nhớ rằng một lần chạy xanh không chứng minh được code đúng, nên các công cụ như jcstress hoặc vòng lặp stress còn đáng giá hơn một test đơn lẻ. Nói chung, chập chờn đến từ trạng thái dùng chung, từ thời gian, từ thứ tự chạy, từ mạng thật và từ việc chờ không giới hạn; cách sửa là cô lập và làm cho mọi thứ tất định, chứ không phải gắn thêm một annotation retry.</p></details>
...
<details class="viet-answer"><summary>🇻🇳 Đáp án (Tiếng Việt)</summary><p>Unit test không chứng minh được bất cứ điều gì xảy ra bên ngoài JVM của bạn: SQL có đúng không, ràng buộc và khóa hoạt động ra sao, mapping JPA và lazy loading thế nào, định dạng serialize, ranh giới transaction và rollback, hay một message broker thật cư xử ra sao. Những thứ đó cần integration test chạy trên <strong>engine thật</strong>, và đó chính là điều Testcontainers mang lại — một Postgres, Kafka hay Redis dùng một lần rồi bỏ, được Docker khởi động riêng cho lần chạy test đó. Cũng vì vậy mà thói quen cũ lấy H2 thay cho Postgres bị khuyến cáo: khác dialect, khác cả ngữ nghĩa ràng buộc, dẫn tới test thì xanh mà production vẫn hỏng. Đi kèm là hai vấn đề kỹ thuật: <strong>chuẩn bị dữ liệu</strong> — chỉ tạo đúng những gì test cần, thông qua builder hoặc một fixture nhỏ — và <strong>cô lập giữa các test</strong> — bằng cách rollback transaction sau mỗi test, truncate bảng giữa các lần chạy, hoặc cho mỗi test một bộ dữ liệu có khóa riêng. Hãy dùng chung container cho cả class hoặc cả bộ test thay vì bật lại cho từng test, và giữ tầng này thật mỏng: mỗi test tốn cả giây, nên nó chỉ nên phủ các ranh giới, còn luật nghiệp vụ thì đã có unit test lo.</p></details>
...
<details class="viet-answer"><summary>🇻🇳 Đáp án (Tiếng Việt)</summary><p>Coverage đo phần code đã được <strong>chạy qua</strong> khi test, chứ không đo hành vi đã thật sự được <strong>kiểm chứng</strong>, nên nó là một tín hiệu hữu ích nhưng lại là một mục tiêu tồi. Line coverage chỉ nói rằng một dòng đã được chạy; branch coverage nói rằng cả hai nhánh của mỗi điều kiện đều đã chạy nên trung thực hơn; nhưng một test không có lấy một assertion nào vẫn được tính là phủ đủ — đó là lý do những đội chạy theo con số phần trăm cuối cùng chỉ có một đống test chạy qua mọi thứ mà chẳng chứng minh được gì. <strong>Mutation testing</strong> lấp đúng khoảng trống đó: công cụ như PIT cố tình sửa bytecode — đảo một phép so sánh, bỏ một lời gọi, đổi giá trị trả về — rồi cho bạn biết bộ test "giết" được bao nhiêu đột biến, tức là đo chất lượng assertion chứ không đo mức độ chạy qua. Trong thực tế, hãy theo dõi coverage như một xu hướng, chỉ đặt ngưỡng bắt buộc cho phần code mới hoặc code vừa sửa, loại trừ code sinh tự động và code cấu hình, nhìn vào những nhánh chưa phủ thay vì nhìn con số, và áp dụng mutation testing cho các module quan trọng, nơi một lỗi lọt lưới sẽ phải trả giá đắt.</p></details>
...
<details class="viet-answer"><summary>🇻🇳 Đáp án (Tiếng Việt)</summary><p>TDD là một vòng lặp thiết kế chứ không phải một chính sách kiểm thử: bạn viết một test đang fail để mô tả hành vi cần có tiếp theo, viết vừa đủ code để nó pass, rồi refactor với test làm lưới an toàn, và cứ thế lặp lại theo từng bước rất nhỏ. Giá trị thật của nó không nằm ở coverage mà ở sức ép lên thiết kế — code viết theo lối test-first thường có các unit nhỏ, phụ thuộc được tiêm vào và ranh giới rõ ràng, đơn giản vì viết kiểu khác thì test sẽ rất khó viết. Hai trường phái khác nhau ở bước ở giữa: phái <strong>classicist (Chicago)</strong> dựng object thật rồi assert vào trạng thái, để thiết kế dần lộ ra qua các bước refactor; còn phái <strong>mockist (London)</strong> mock hết collaborator và dẫn dắt thiết kế từ ngoài vào theo các tương tác. TDD phát huy nhất khi luật nghiệp vụ phức tạp hoặc yêu cầu đã rõ ràng và chính xác, và ít tác dụng nhất với việc mang tính thăm dò, dựng giao diện hay code kết nối đơn giản — nên câu trả lời trung thực của một senior là: đây chỉ là một công cụ trong nhiều công cụ, hãy dùng ở nơi mà vòng phản hồi nó mang lại xứng với chi phí bỏ ra.</p></details>
...
<details class="viet-answer"><summary>🇻🇳 Đáp án (Tiếng Việt)</summary><p>Code test cũng là code production, chỉ khác mục đích, nên phải được chăm sóc ngang bằng — và có thêm một quy tắc riêng: test nên <strong>DAMP</strong> (viết rõ ràng, đọc là hiểu) hơn là DRY đến mức cực đoan, vì một test giấu phần chuẩn bị dữ liệu sau ba tầng helper dùng chung thì không còn đọc được như một bản đặc tả nữa. Những cách giữ cho một bộ test lớn còn khỏe mạnh: dùng test data builder với giá trị mặc định hợp lý để mỗi test chỉ phải nêu đúng thứ nó quan tâm; đặt tên test theo hành vi để khi build đỏ là chẩn đoán được ngay từ báo cáo, khỏi cần mở file; và một luật cứng là trong test không được có logic. Mối nguy lớn nhất là <strong>test dễ vỡ</strong>: test bám vào chi tiết cài đặt, mock bị verify quá tay, fixture dùng chung có thể bị sửa, và assert cả vào những đầu ra chẳng liên quan — tất cả đều làm test fail khi refactor, rồi dần dạy cả đội mất niềm tin vào bộ test. Hãy review test nghiêm túc như review code production, xóa những test trùng lặp hoặc không assert gì, và giữ cho bộ test nhanh vẫn luôn nhanh, vì một bộ test chạy mười phút thì rồi sẽ chẳng ai chạy nữa.</p></details>
