...
<details class="viet-answer"><summary>🇻🇳 Đáp án (Tiếng Việt)</summary><p>TypeScript là JavaScript được bổ sung thêm một tầng kiểu tĩnh và được biên dịch xuống thành JavaScript thuần. Nó bắt lỗi kiểu ngay tại thời điểm biên dịch và mang lại công cụ hỗ trợ tốt hơn như autocomplete, refactor an toàn, và code tự mô tả. Cái giá phải trả chủ yếu là thêm một bước build và một chút thời gian làm quen, nên nó phù hợp nhất với code được cả team bảo trì lâu dài. Kiểu chỉ tồn tại lúc biên dịch và biến mất khi chạy, nên dữ liệu bên ngoài như response từ API vẫn cần được kiểm tra lại.</p></details>
<ul>
...
<details class="viet-answer"><summary>🇻🇳 Đáp án (Tiếng Việt)</summary><p>Nguyên tắc thường gặp là dùng <code>interface</code> cho hình dạng của object và class, còn <code>type</code> cho mọi thứ còn lại. Interface hỗ trợ declaration merging và hợp với các contract của class, trong khi type alias có thể biểu diễn union, intersection, tuple và mapped type mà interface không làm được. Với object thông thường thì hai cái gần như thay thế cho nhau, nên điều quan trọng nhất là giữ nhất quán. Vì interface có thể merge, một interface public có thể bị code khác mở rộng thêm, điều này vừa có thể hữu ích vừa có thể tiềm ẩn rủi ro.</p></details>
...
<details class="viet-answer"><summary>🇻🇳 Đáp án (Tiếng Việt)</summary><p>Generics cho phép một đoạn code tái sử dụng làm việc được với nhiều kiểu khác nhau mà vẫn giữ được mối liên hệ giữa input và output, nên một hàm nhận <code>T[]</code> sẽ trả về <code>T</code> thay vì <code>any</code>. Dùng <code>any</code> là vứt bỏ kiểu đi và ngừng kiểm tra, còn generic giữ nguyên kiểu thật xuyên suốt toàn bộ signature. Tốt nhất nên để TypeScript tự suy luận tham số kiểu, và thêm ràng buộc như <code>K extends keyof T</code> khi code cần truy cập thành viên một cách an toàn. Một quy tắc hay là nếu một tham số kiểu chỉ xuất hiện một lần thì có lẽ nó không cần phải là generic.</p></details>
...
<details class="viet-answer"><summary>🇻🇳 Đáp án (Tiếng Việt)</summary><p>Union và intersection dùng để dựng nên các kiểu tổng hợp, còn narrowing là cách sử dụng một union một cách an toàn về sau. Union nghĩa là giá trị hoặc là A hoặc là B, nên chỉ những thành viên chung của cả hai mới dùng được cho tới khi được narrow, trong khi intersection gộp A và B thành một hình dạng duy nhất. Narrowing thu hẹp một union bên trong một nhánh bằng <code>typeof</code>, <code>instanceof</code>, <code>in</code>, hoặc một trường tag chung. Một discriminated union với trường <code>kind</code> chung cộng với default <code>assertNever</code> giúp trình biên dịch kiểm tra được mọi trường hợp, nên khi thiếu một biến thể mới thì sẽ thành lỗi biên dịch.</p></details>
...
<details class="viet-answer"><summary>🇻🇳 Đáp án (Tiếng Việt)</summary><p>Đây là ba ranh giới của hệ thống kiểu. <code>any</code> tắt hoàn toàn việc kiểm tra kiểu và lan sang mọi thứ nó chạm vào, nên tốt nhất là tránh dùng. <code>unknown</code> là phiên bản an toàn và bắt buộc phải narrow trước khi dùng, phù hợp với các giá trị như kết quả của <code>JSON.parse</code> hoặc input bên ngoài. <code>never</code> biểu diễn một giá trị không thể tồn tại, và công dụng chính của nó là kiểm tra tính đầy đủ, khi một <code>assertNever</code> trong default của switch sẽ không biên dịch được lúc có thành viên union mới được thêm vào.</p></details>
<ul>
...
<details class="viet-answer"><summary>🇻🇳 Đáp án (Tiếng Việt)</summary><p>Utility type là những bộ biến đổi generic dùng để dựng một hình dạng từ một hình dạng khác thay vì phải viết tay các interface song song. Những cái phổ biến gồm <code>Omit</code> cho payload tạo mới không có <code>id</code>, <code>Partial</code> cho payload cập nhật, <code>Pick</code> cho một view nhỏ, và <code>Record</code> cho dictionary. Việc dẫn xuất từ một kiểu nguồn duy nhất nghĩa là khi đổi tên một trường thì mọi kiểu dẫn xuất đều cập nhật theo, tránh được các bản sao cũ bị lệch. Một điểm cần lưu ý là <code>Partial</code> và <code>Readonly</code> chỉ tác động ở một tầng nông, nên các object lồng nhau cần một <code>DeepPartial</code> đệ quy.</p></details>
...
<details class="viet-answer"><summary>🇻🇳 Đáp án (Tiếng Việt)</summary><p>Mapped type biến đổi mọi thuộc tính của một hình dạng, conditional type là một câu <code>if</code> ở cấp độ kiểu được viết dưới dạng <code>T extends U ? X : Y</code>, còn <code>infer</code> trích một kiểu ra từ một mẫu. Kết hợp lại, chúng tạo nên nhiều kiểu của thư viện như <code>ReturnType</code> và <code>Awaited</code>. Một điểm tinh tế là tính phân phối: một tham số kiểu trần sẽ phân phối trên một union, điều này rất mạnh nhưng có thể gây bất ngờ khi kiểm tra <code>never</code>. Các công cụ này hữu ích để tính ra kiểu từ một nguồn sự thật, nhưng các conditional lồng sâu sẽ trở nên khó đọc.</p></details>
...
<details class="viet-answer"><summary>🇻🇳 Đáp án (Tiếng Việt)</summary><p>Type guard là bất cứ thứ gì thu hẹp một kiểu rộng xuống một kiểu cụ thể hơn trong một phạm vi. TypeScript tự hiểu <code>typeof</code>, <code>instanceof</code>, và <code>in</code>, còn một guard tự viết thì dùng kiểu trả về được viết dưới dạng một predicate như <code>pet is Fish</code>. Kiểu trả về bắt buộc phải là predicate chứ không phải <code>boolean</code> thuần, nếu không trình biên dịch vẫn chạy phép kiểm tra nhưng không narrow. Một predicate được tin tưởng mà không kiểm tra lại, nên với dữ liệu bên ngoài thì dùng một schema validator như Zod tự sinh ra guard sẽ an toàn hơn.</p></details>
...
<details class="viet-answer"><summary>🇻🇳 Đáp án (Tiếng Việt)</summary><p>Mặc định TypeScript mở rộng các literal vì nó giả định giá trị có thể thay đổi, nên một thuộc tính sẽ được suy ra là <code>string</code> thay vì đúng giá trị của nó. <code>as const</code> làm ngược lại: nó biến giá trị thành <code>readonly</code> sâu, narrow từng literal về đúng kiểu của nó, và biến mảng thành tuple <code>readonly</code>. Một ứng dụng phổ biến là dùng thay cho enum, khi một object <code>as const</code> cộng với <code>typeof OBJ[keyof typeof OBJ]</code> cho ra cả giá trị lúc chạy lẫn một union literal chính xác mà không sinh thêm code. Một điểm cần lưu ý là kết quả là <code>readonly</code> sâu, nên phải sao chép trước khi truyền vào nơi cần một mảng có thể thay đổi.</p></details>
...
<details class="viet-answer"><summary>🇻🇳 Đáp án (Tiếng Việt)</summary><p>Enum là một trong số ít tính năng của TypeScript sinh ra code lúc chạy, và thường bị tránh dùng. Numeric enum thêm một reverse mapping làm tăng kích thước và, trước TS 5.0, còn chấp nhận bất kỳ số nào. <code>const enum</code> nội tuyến các giá trị và làm hỏng các transpiler xử lý từng file như Babel và esbuild. Lựa chọn mặc định tốt hơn là một union các string literal cho trường hợp đơn giản, hoặc một object <code>as const</code> khi cần cả giá trị lúc chạy lẫn kiểu, vì cả hai đều dùng JavaScript thuần và tree-shake tốt.</p></details>
<pre>// Numeric enum
enum Direction {
...
<details class="viet-answer"><summary>🇻🇳 Đáp án (Tiếng Việt)</summary><p>Một file <code>.d.ts</code> chỉ chứa thông tin kiểu mà không có phần cài đặt, và nó mô tả hình dạng của JavaScript vốn không có kiểu riêng. Phần lớn thời gian, kiểu đến từ việc cài một package <code>@types</code> từ DefinitelyTyped. Bạn cần một declaration viết tay khi một thư viện không kèm kiểu, hoặc để bổ sung cho các kiểu toàn cục chẳng hạn như thêm một thuộc tính vào <code>Window</code>. Cách xử lý nhanh cho một package không có kiểu là một dòng <code>declare module 'libname'</code>, khiến các export của nó thành <code>any</code>.</p></details>
...
<details class="viet-answer"><summary>🇻🇳 Đáp án (Tiếng Việt)</summary><p>Thiết lập quan trọng nhất là <code>strict: true</code>, nên bật cho mọi dự án mới, vì nó gom nhóm các kiểm tra như <code>strictNullChecks</code> và <code>noImplicitAny</code> vốn bắt được hầu hết bug thực sự. Bật nó cho một codebase cũ thì khó hơn, và ở đó có thể bật từng kiểm tra một. Các thiết lập quan trọng khác gồm <code>target</code> và <code>lib</code> khớp với môi trường chạy, <code>moduleResolution</code> đặt là bundler cho các cấu hình hiện đại, và <code>skipLibCheck</code> để build nhanh hơn. Cũng đáng thêm <code>noUncheckedIndexedAccess</code>, vốn không nằm trong strict mode.</p></details>
<pre>// tsconfig.json
{
  "compilerOptions": {
...
<details class="viet-answer"><summary>🇻🇳 Đáp án (Tiếng Việt)</summary><p>Decorator là những hàm dùng để chú thích và sửa đổi class, method hoặc thuộc tính, và chúng là cốt lõi của NestJS, Angular và TypeORM. Có hai phiên bản: dạng cũ <code>experimentalDecorators</code> mà các framework đó vẫn dùng, và dạng chuẩn Stage 3 native trong TS 5.0, và hình dạng của chúng khác nhau. Điều đầu tiên cần kiểm tra là codebase đang dùng phiên bản nào, vì các signature không giống nhau. Chúng phù hợp với các mối quan tâm xuyên suốt như logging hay metadata cho route, nhưng chúng thêm hành vi ẩn, nên logic bên trong nên giữ đơn giản.</p></details>
...
<details class="viet-answer"><summary>🇻🇳 Đáp án (Tiếng Việt)</summary><p>Khi <code>strictNullChecks</code> được bật, <code>null</code> và <code>undefined</code> là các kiểu riêng và không thể lọt vào một <code>string</code>, điều này ngăn được nhiều lỗi crash do đọc thuộc tính của undefined. Công cụ chính là optional chaining <code>?.</code> và nullish coalescing <code>??</code>, và cần nhớ rằng <code>??</code> chỉ dùng giá trị dự phòng khi gặp null hoặc undefined, trong khi <code>||</code> còn dùng dự phòng cho cả chuỗi rỗng và số không. Non-null assertion <code>!</code> âm thầm loại bỏ sự an toàn này, nên narrow bằng một guard hoặc xử lý trực tiếp trường hợp null sẽ tốt hơn.</p></details>
...
<details class="viet-answer"><summary>🇻🇳 Đáp án (Tiếng Việt)</summary><p>TypeScript có type assertion chứ không phải cast, vì không có gì thay đổi lúc chạy và assertion chỉ bảo trình biên dịch hãy tin vào giá trị. Điều này khiến <code>as</code> tiềm ẩn rủi ro, vì một assertion sai sẽ gây crash lúc chạy mà không cảnh báo, còn một double assertion qua <code>unknown</code> thì bỏ qua toàn bộ kiểm tra. Tốt nhất nên coi nó là biện pháp cuối cùng. Lựa chọn an toàn hơn là một type guard thực sự khi cần an toàn lúc chạy, và <code>satisfies</code> khi mục tiêu là kiểm tra một literal so với một kiểu mà không làm rộng kiểu ra.</p></details>
...
<details class="viet-answer"><summary>🇻🇳 Đáp án (Tiếng Việt)</summary><p><code>extends</code> nghĩa là kế thừa và cung cấp phần cài đặt, hoặc trong generic thì nó ràng buộc một tham số kiểu, còn <code>implements</code> là một lời cam kết thỏa mãn một contract mà trong đó mọi thành viên đều phải được cung cấp. Một class chỉ có thể extends một class nhưng có thể implements nhiều interface, đây là cách TypeScript xử lý kế thừa đơn. Dùng <code>implements</code> với các interface nhỏ mang lại sự linh hoạt, còn <code>extends</code> phù hợp khi thực sự có code chung để chia sẻ. Lưu ý rằng <code>implements</code> không thêm code nào và chỉ là một kiểm tra lúc biên dịch xem hình dạng có khớp không.</p></details>
<ul>
...
<details class="viet-answer"><summary>🇻🇳 Đáp án (Tiếng Việt)</summary><p><code>satisfies</code> kiểm tra một giá trị có khớp với một kiểu không mà không làm rộng kiểu ra, nên nó cho ta cả sự kiểm tra của một annotation lẫn kiểu suy luận chính xác. Một trường hợp phổ biến là một object config hay palette, khi một annotation <code>Record</code> tường minh sẽ làm mất các kiểu cụ thể theo từng key, nhưng <code>satisfies</code> giữ chúng lại, biết rằng <code>red</code> là một mảng số còn <code>green</code> là một chuỗi mà vẫn bắt được các key viết sai. Nó khắc phục thói quen cũ là gắn <code>as</code> lên object literal, vốn chỉ khiến trình biên dịch bị đánh lừa. Một quy tắc hay là ưu tiên <code>satisfies</code> trước <code>as</code>.</p></details>
...
<details class="viet-answer"><summary>🇻🇳 Đáp án (Tiếng Việt)</summary><p><code>void</code> nghĩa là bên gọi không nên dựa vào giá trị trả về, <code>undefined</code> nghĩa là hàm thực sự trả về undefined, còn <code>never</code> nghĩa là hàm không bao giờ trả về vì nó throw hoặc lặp vô hạn. Một cái bẫy phổ biến là một kiểu callback <code>() =&gt; void</code> vẫn chấp nhận một hàm có trả về giá trị, vì giá trị trả về đơn giản là bị bỏ qua, đó là lý do <code>Array.forEach</code> chấp nhận các callback như vậy. <code>never</code> là cái hữu ích cho việc kiểm tra tính đầy đủ. Vậy nên <code>void</code> nên được hiểu là giá trị trả về bị bỏ đi, chứ không phải là không được trả về gì cả.</p></details>
...
<details class="viet-answer"><summary>🇻🇳 Đáp án (Tiếng Việt)</summary><p>Template literal type là việc dựng chuỗi ở cấp độ kiểu, khi các kiểu string-literal được ghép lại bằng cách nhúng các literal type khác vào, và việc bắt chéo các union tạo ra mọi tổ hợp. Chúng quan trọng vì nhiều chuỗi trong API mang cấu trúc, chẳng hạn tên hàm xử lý sự kiện, giá trị CSS, tham số route, và các key có tiền tố, và điều này cho phép trình biên dịch kiểm tra và thậm chí phân tích chúng thay vì coi chúng như <code>string</code> thuần. Kết hợp với <code>infer</code>, chúng có thể rút dữ liệu ra khỏi một kiểu chuỗi, đây là cách các thư viện kiểm tra kiểu cho đường dẫn route. Một điểm cần lưu ý là sự bùng nổ tổ hợp, vì bắt chéo vài union lớn có thể vượt quá giới hạn độ phức tạp của trình biên dịch.</p></details>
...
<details class="viet-answer"><summary>🇻🇳 Đáp án (Tiếng Việt)</summary><p>Declaration merging là quy tắc rằng hai interface cùng tên sẽ gộp thành một, còn module augmentation áp dụng điều này cho các kiểu của bên thứ ba hoặc kiểu toàn cục, chẳng hạn thêm một thuộc tính <code>user</code> vào <code>Request</code> của Express để nó được gán kiểu trong mọi middleware. Điều này chỉ hoạt động với <code>interface</code> và <code>namespace</code>, không bao giờ với <code>type</code> alias, đây là một lý do chính để ưu tiên <code>interface</code> cho các hình dạng có thể mở rộng. Vì nó mạnh nhưng ngầm định, các augmentation nên nằm trong một file <code>.d.ts</code> đặt tên rõ ràng để người khác tìm được nguồn gốc của một thuộc tính được thêm vào.</p></details>
...
<details class="viet-answer"><summary>🇻🇳 Đáp án (Tiếng Việt)</summary><p>TypeScript dùng kiểu theo cấu trúc, nên một object có thêm thuộc tính thường vẫn gán được, nhưng một object literal mới được truyền trực tiếp thì phải qua thêm một kiểm tra thuộc tính dư thừa vốn báo lỗi các key lạ, chủ yếu để bắt lỗi gõ sai ở các thuộc tính optional. Một khi literal đó được gán vào một biến trước, nó mất đi tính "tươi" và kiểm tra này không còn chạy nữa, nên cùng một object có thể qua hoặc không qua tùy theo việc nó có đi qua một biến hay không. Để giữ sự an toàn đó trên một biến, dùng <code>satisfies</code>, vốn chạy lại toàn bộ kiểm tra mà không làm rộng kiểu.</p></details>
...
<details class="viet-answer"><summary>🇻🇳 Đáp án (Tiếng Việt)</summary><p>Kiểu theo cấu trúc nghĩa là tính tương thích phụ thuộc vào hình dạng chứ không phải vào tên được khai báo, nên hai interface có cùng thành viên là thay thế được cho nhau, khác với các ngôn ngữ nominal như Java hay C#. Điều này gây rắc rối với các ID trong domain: nếu <code>userId</code> và <code>orderId</code> đều là <code>string</code>, việc tráo chúng cho nhau vẫn biên dịch được và có thể làm hỏng dữ liệu. Một branded type khắc phục điều này bằng cách giao <code>string</code> với một thuộc tính giả <code>readonly __brand</code> vốn không tồn tại lúc chạy, và việc tạo ra được dồn qua một factory. Nó không tốn chi phí lúc chạy, và Zod cung cấp cùng ý tưởng đó qua <code>.brand</code>.</p></details>
...
<details class="viet-answer"><summary>🇻🇳 Đáp án (Tiếng Việt)</summary><p>Một conditional type sẽ phân phối khi kiểu được kiểm tra là một tham số kiểu trần, nên khi áp lên một union nó chạy một lần cho mỗi thành viên rồi gộp kết quả lại. Chính quy tắc này tạo nên <code>Exclude</code>, <code>Extract</code> và <code>NonNullable</code>. Một cái bẫy phổ biến là <code>never</code>, vốn là union rỗng, nên phân phối trên nó không tạo ra gì cả, và một kiểm tra <code>IsNever</code> ngây thơ sẽ trả về <code>never</code> thay vì <code>true</code>. Cách khắc phục là bọc cả hai vế trong một tuple, <code>[T] extends [U]</code>, điều này tắt phân phối đi và cũng giữ một union nguyên vẹn như một kiểu mảng duy nhất.</p></details>
...
<details class="viet-answer"><summary>🇻🇳 Đáp án (Tiếng Việt)</summary><p>Mọi utility có sẵn đều là một mapped type một dòng, và việc tự viết chúng cho thấy sự hiểu biết thực sự về <code>keyof</code>, indexed access, cùng các bổ ngữ <code>+</code> và <code>-</code> thay vì chỉ học thuộc một API. <code>Partial</code> thêm <code>?</code>, <code>Required</code> dùng <code>-?</code> để bỏ nó đi, còn <code>Mutable</code> dùng <code>-readonly</code>. Đổi tên key bằng <code>as</code> có thể đổi tên các key, dựng một interface getter, hoặc lọc các thuộc tính bằng cách ánh xạ các key không mong muốn thành <code>never</code>, đây là cách Prisma và tRPC tính ra kiểu từ một nguồn duy nhất. Các homomorphic mapped type còn giữ nguyên các bổ ngữ <code>?</code> và <code>readonly</code> gốc.</p></details>
...
<details class="viet-answer"><summary>🇻🇳 Đáp án (Tiếng Việt)</summary><p>Ba vấn đề cụ thể khiến enum đáng bị tránh. Numeric enum thêm một reverse mapping, nên <code>Object.keys</code> trả về gấp đôi số mục và bundle phải gánh thêm phần nặng. Trước TS 5.0, một numeric enum chấp nhận bất kỳ số nào, điều này không an toàn với các giá trị đến từ JSON. Và <code>const enum</code> nội tuyến ngay tại chỗ gọi, vốn cần thông tin toàn chương trình, nên nó làm hỏng các transpiler xử lý từng file như Babel, esbuild và swc, đồng thời trói người dùng thư viện vào các thiết lập của trình biên dịch. Lựa chọn tốt hơn là union các string literal hoặc một object <code>as const</code>, vốn dùng JavaScript thuần và tree-shake tốt.</p></details>
...
<details class="viet-answer"><summary>🇻🇳 Đáp án (Tiếng Việt)</summary><p>TypeScript đánh đổi tính đúng đắn lấy sự dễ dùng, nên strict mode không phải là một sự chứng minh và tồn tại vài lỗ hổng. Một lỗ hổng là tham số của method viết ngắn gọn vẫn giữ tính bivariant ngay cả khi bật <code>strictFunctionTypes</code>, trong khi chỉ cú pháp function-property mới được kiểm tra chặt, nên một callback được gán kiểu lỏng lẻo có thể nhận sai sự kiện và gây crash. Một lỗ hổng khác là tính covariant của mảng, khi một <code>Dog[]</code> gán được vào một <code>Animal[]</code> dù sau đó một <code>Cat</code> có thể được push vào và gây lỗi lúc chạy. Cách phòng vệ chính là dùng cú pháp function-property cho callback, nhận <code>readonly T[]</code> cho đầu vào, và kiểm tra dữ liệu tại các ranh giới tin cậy.</p></details>
