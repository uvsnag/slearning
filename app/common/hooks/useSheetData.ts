...
/**
 * Update two adjacent cells in a Google Sheet with two values
 * Finds the first empty row in the range and inserts/updates there
 * @param {any} gapi - Google API instance
 * @param {Function} callback - Callback function (success, error)
 * @param {String} range - Full range string (e.g., 'AUTO!A2:C500')
 * @param {String} value1 - First value to save
 * @param {String} value2 - Second value to save
 */
const updateGGSheetTwoValues = async (
  gapi: any,
  callback: any,
  range: string,
  value1: string,
...
export const ggSheetProcess = async (
  callback: any,
  sheet: string,
...
// const onLoad = (data: DataItem[] | null, error: Error | null): void => {
//   if (data) {
//     // const result = data;
//     const arr: DataItem[] = [];
...
//   }
// };
export function onRemoveStoreItem(currEng: string, callback: () => void, sheet: string): void {
  if (sheet?.startsWith(STORE_ALIAS)) {
    const storeDataString = localStorage.getItem(sheet);
 ...
 * @param {String} range - Full range string from SHEET_AUTO (e.g., 'AUTO!A2:C500')
 * @param {String} value1 - First value to save
 * @param {String} value2 - Second value to save
 */
export const ggSheetProcessTwoValues = async (
  callback: any,
  range: string,
  value1: string,
  value2: string,
  ...
  gapi.load('client:auth2', async () => {
    ...
    updateGGSheetTwoValues(gapi, callback, range, value1, value2);
  });
};
